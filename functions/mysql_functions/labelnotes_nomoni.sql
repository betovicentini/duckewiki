CREATE FUNCTION labelnotes_nomoni(speclinkid int(10), pltid int(10), formid int(10), printn BOOLEAN, printclass BOOLEAN) RETURNS text CHARSET utf8
BEGIN
...
DECLARE cur1 CURSOR FOR SELECT * FROM ((SELECT TraitTipo,TraitName,Traits.PathName,TraitVariation,Traits_variation.TraitUnit,formlist.Ordem FROM Traits_variation JOIN Traits USING(TraitID) JOIN FormulariosTraitsList as formlist ON formlist.TraitID=Traits.TraitID WHERE Traits_variation.PlantaID=pltid AND formlist.FormID=formid)) as firstab ORDER BY firstab.Ordem;
DECLARE cur1a CURSOR FOR SELECT * FROM ((SELECT TraitTipo,TraitName,Traits.PathName,TraitVariation,Traits_variation.TraitUnit,0 as Ordem FROM Traits_variation JOIN Traits USING(TraitID) WHERE Traits_variation.PlantaID=pltid)) as firstaba ORDER BY firstaba.PathName;
...
DECLARE cur3 CURSOR FOR SELECT * FROM ((SELECT TraitTipo,TraitName,Traits.PathName,TraitVariation,Traits_variation.TraitUnit, formlist.Ordem  FROM Traits_variation JOIN Traits USING(TraitID) JOIN FormulariosTraitsList as formlist ON formlist.TraitID=Traits.TraitID WHERE Traits_variation.PlantaID=pltid AND formlist.FormID=formid) UNION (SELECT TraitTipo,TraitName,Traits.PathName,TraitVariation,Traits_variation.TraitUnit, formlist.Ordem  FROM Traits_variation JOIN Traits USING(TraitID) JOIN FormulariosTraitsList as formlist ON formlist.TraitID=Traits.TraitID WHERE Traits_variation.EspecimenID=speclinkid AND formlist.FormID=formid)) as newtab ORDER BY newtab.Ordem;
DECLARE cur3a CURSOR FOR SELECT * FROM ((SELECT TraitTipo,TraitName,Traits.PathName,TraitVariation,Traits_variation.TraitUnit, 0 as Ordem FROM Traits_variation JOIN Traits USING(TraitID) WHERE Traits_variation.PlantaID=pltid ORDER BY Traits.PathName) UNION  (SELECT TraitTipo,TraitName,Traits.PathName,TraitVariation,Traits_variation.TraitUnit, 0 as Ordem FROM Traits_variation JOIN Traits USING(TraitID) WHERE Traits_variation.EspecimenID=speclinkid ORDER BY Traits.PathName)) as newtaba ORDER BY newtaba.PathName;DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = 1;
