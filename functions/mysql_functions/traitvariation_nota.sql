...
DECLARE cur1 CURSOR FOR SELECT TraitTipo,TraitName,Traits.PathName,TraitVariation,Traits_variation.TraitUnit FROM Traits_variation JOIN Traits USING(TraitID) WHERE Traits_variation.EspecimenID=linkid AND Traits.FormulariosIDS LIKE CONCAT('%formid_',formid,'%') ORDER BY Traits.PathName;
