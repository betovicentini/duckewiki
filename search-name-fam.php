...
HTMLheaders('');
		...
		//taxonomic summary
		$qq = "SELECT DISTINCT Genero FROM Tax_Generos JOIN Tax_Familias USING(FamiliaID) WHERE Familia='".$nn."'";
		$rr = mysql_query($qq,$conn);
		$ngen = mysql_numrows($rr);
		
		$qq = "SELECT DISTINCT Genero,Especie FROM Tax_Especies JOIN Tax_Generos USING(GeneroID) JOIN Tax_Familias USING(FamiliaID) WHERE Familia='".$nn."'";
		$rr = mysql_query($qq,$conn);
		$nsp = mysql_numrows($rr);

		$qq = "SELECT DISTINCT Genero,Especie,InfraEspecie FROM Tax_InfraEspecies JOIN Tax_Especies USING(EspecieID) JOIN Tax_Generos USING(GeneroID) JOIN Tax_Familias USING(FamiliaID) WHERE Familia='".$nn."'";
		$rr = mysql_query($qq,$conn);
		$ninfsp = mysql_numrows($rr);
		
		$qq = "SELECT * FROM Especimenes JOIN Identidade USING(DetID) JOIN Tax_Familias ON Identidade.FamiliaID=Tax_Familias.FamiliaID WHERE Familia='".$nn."'";
		$rr = mysql_query($qq,$conn);
		$ncols = mysql_numrows($rr); 
		
		$qq = "SELECT * FROM Plantas JOIN Identidade USING(DetID) JOIN Tax_Familias ON Identidade.FamiliaID=Tax_Familias.FamiliaID WHERE Familia='".$nn."'";
		$rr = mysql_query($qq,$conn);
		$nplantas = mysql_numrows($rr); 
		
		//imagens
		$qq = "(SELECT TraitVariation as imagens FROM Traits_variation JOIN Traits USING (TraitID) JOIN Especimenes  ON Traits_variation.EspecimenID=Especimenes.EspecimenID JOIN Identidade USING(DetID) JOIN Tax_Familias ON Identidade.FamiliaID=Tax_Familias.FamiliaID WHERE TraitTipo='Variavel|Imagem' AND Familia='".$nn."') UNION (SELECT TraitVariation as imagens FROM Traits_variation JOIN Traits USING (TraitID) JOIN Plantas ON Traits_variation.PlantaID=Plantas.PlantaID JOIN Identidade USING(DetID) JOIN Tax_Familias ON Identidade.FamiliaID=Tax_Familias.FamiliaID WHERE TraitTipo='Variavel|Imagem' AND Familia='".$nn."') UNION (SELECT TraitVariation as imagens FROM Traits_variation JOIN Traits USING (TraitID) JOIN Tax_Familias ON Traits_variation.FamiliaID=Tax_Familias.FamiliaID WHERE TraitTipo='Variavel|Imagem' AND Familia='".$nn."')";
		$rr = mysql_query($qq,$conn);
		...
		if ($rr) {
			...
			if ($valid==1) {
				...
				foreach ($ff as $k => $vv) {
					$fid = explode("|",$vv);
					$ffid = $fid[1];
					$qq = "SELECT Familia FROM Tax_Familias WHERE FamiliaID='".$ffid."'";
					$rw = mysql_query($qq,$conn);
					$rww = mysql_fetch_assoc($rw);
					$sinome = "<a href=\"?nn=".$rww['Familia']."\">".$rww['Familia']."</a>";
					if (!empty($sino)) {					
						$sino = $sino." | ".$sinome;
					} else {
						$sino = $sinome;
				...
				$sino = "<a href=\"?nn=".$roww['Familia']."\">".$roww['Familia']."</a>";
			}
	}
		...
		$famurl = "http://www.mobot.org/mobot/research/apweb/orders/".strtolower($ordem)."web.htm#".$nn;
		$resurl = file_exists($orderurl);
		if (!$resurl) {
			$orderurl = "http://www.mobot.org/mobot/research/apweb/orders/".strtolower($ordem)."web2.htm#".$ordem;
			$famurl = "http://www.mobot.org/mobot/research/apweb/orders/".strtolower($ordem)."web2.htm#".$nn;

			$resurl2 = file_exists($orderurl);
			if (!$resurl2) {
				$orderurl = "http://www.mobot.org/mobot/research/apweb/orders/".$ordem.".html#".$ordem;
				$famurl = "http://www.mobot.org/mobot/research/apweb/orders/".$ordem.".html#".$nn;
		...
		<tr >
			<td  colspan=2 class='tabhead'>$nn</td>
		</tr>
		<tr>";
			if ($valid==1) {
			...
			echo "
				<tr>
					...
					<td class='tdsmallbold'>$nn&nbsp;$famautor ($orderlink)&nbsp;";
					if ($valid==0) {
						echo "[$namevalid]";					
					}
					echo "</td>
		...
		<tr>			
				<th class='small'>".GetLangVar('namegenus')."</th>
		...
		if ($listgen==1 && $ngen>0) {
			echo "
				...
				<tr><td colspan=2><table class='sortable autostripe' align='center' width='99%'>
				<thead>
					<tr>
				...
				<tbody>";			
				$qq = "SELECT * FROM Tax_Generos JOIN Tax_Familias USING(FamiliaID) WHERE Familia='".$nn."' ORDER BY Genero";
				$rr = mysql_query($qq,$conn);
				$bgi=1;
				while ($gen = mysql_fetch_assoc($rr)) {
						$gg = $gen['Genero'];
						$geau = $gen['GeneroAutor'];
						echo "<tr";
						if ($bgi % 2 != 0){ echo ' class=odd ' ;}
							$bgi++;
							echo 	"><td><a href='search-name-gen.php?nn=$gg'><b><i>".$gg."</i></b></a>&nbsp;$geau</td>
							</tr>";		
				}
		...
		if ($listsp==1 && $nsp>0) {
			echo "
				...
				$qq = "SELECT * FROM Tax_Especies JOIN Tax_Generos USING(GeneroID) JOIN Tax_Familias USING(FamiliaID) WHERE Familia='".$nn."' ORDER BY Genero,Especie";
				$rr = mysql_query($qq,$conn);
				$bgi=1;
				while ($gen = mysql_fetch_assoc($rr)) {
						$gg = $gen['Genero'];
		...
		if ($listinfsp==1 && $ninfsp>0) {
			echo "
				...
				<tr>
					...
					<tbody >";			
				$qq = "SELECT Genero,Especie,EspecieAutor,Tax_Especies.BasionymAutor as SpBasAu,InfraEspecieNivel as nivel,InfraEspecie,InfraEspecieAutor as autor,Tax_InfraEspecies.BasionymAutor FROM Tax_InfraEspecies JOIN Tax_Especies USING(EspecieID) JOIN Tax_Generos USING(GeneroID) JOIN Tax_Familias USING(FamiliaID) WHERE Familia='".$nn."' ORDER BY Genero,Especie,InfraEspecie";
				$rr = mysql_query($qq,$conn);
				$bgi=1;
				while ($gen = mysql_fetch_assoc($rr)) {
						$gg = $gen['Genero'];
