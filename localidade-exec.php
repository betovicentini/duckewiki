...
if ($enviado=='final') {
	...
	@extract($coord);
	 if ($_SESSION['editando']!=1)  {
	 		if ($parentgazid>0) {
				$check = "SELECT * FROM Gazetteer WHERE Gazetteer='$gazetteer' AND ParentID=".$parentgazid;
			} else {
				$check = "SELECT * FROM Gazetteer WHERE Gazetteer='$gazetteer' AND MunicipioID=".$municipioid;
			}
			//echo $check."<br >";
			...
			if ($update>0) {
				...
				if (!$newgazid) {
					...
					//echo "SIM RODOU AQUI $newgazid";
				}
			}
			if ($erro==0) {
				 echo "<table align='center' class='success' cellpadding=\"5\" cellspacing=0 width='90%'><tr>
				<td>".GetLangVar('sucesso1')."</td></tr></table><br>";
			} else {
				echo "<table align='center' class='erro' cellpadding=\"5\" cellspacing=0 width='90%'><tr>
...
//					<tr><td><b>".GetLangVar('nametipo')."</b></td><td><b>".GetLangVar('namenome')."</b></td><td><b>Latitude</b></td><td><b>Longitude</b></td></tr>";
				...
				<tr>
					...
					<form action=localidade-form.php method='post'>	
						<td align='left'><input type='submit' value=".GetLangVar('namecancel')." class='breset'></td>
					</form>
				</tr>";
...
//						<input type='hidden' name='gazetteertipo' value='$gazetteertipo'>
			...
			if (empty($_POST['doitnocoord']) && empty($latdec) && empty($longdec)) {
				echo "<form action=localidade-exec.php method='post'>	
...
echo "</table><br>";
	...
	if ($erros==0) {
		...
		if ($_SESSION['editando']!=1) {
		echo "<table align='center' class='myformtable' cellpadding='4' cellspacing='0'>
		<thead>
		...
		//echo "<tr><td align='right' ><b>".GetLangVar('namegazetteer')." <i>".GetLangVar('nametipo')."</i></b></td><td align='left'><i>".$row['GazetteerTIPOtxt']."</i></td></tr>";
...
//						<input type='hidden' name='gazetteertipo' value='$gazetteertipo'>
			...
			echo "<form name='myform' action=localidade-exec.php method='post'>	
						<input type='hidden' name='paisid' value='$paisid'>
						<input type='hidden' name='provinciaid' value='$provinciaid'>
						<input type='hidden' name='municipioid' value='$municipioid'>
						<input type='hidden' name='gazetteer' value='$gazetteer'>
...
if (!isset($_POST['enviado']) || $erros>0) {
echo "<br><table align='center' class='myformtable' cellpadding=\"4\">
<thead>
<tr>
<td colspan=100%>";
...
echo "<form enctype='multipart/form-data' action='localidade-exec.php' method='post'>
...
//		<td align='left'>
//		<input type='text' id='gazetteertipo' name='gazetteertipo' value='$gazetteertipo'>
//		</td>
//	</tr>
//	</table>
//</td></tr>";
...
</td><td align='left'>$gazpathname</td>
</tr></table>
</td></tr>";
...
echo "<tr bgcolor = $bgcolor>
	<td colspan='100%' align='left' class='tdformnotes'>&nbsp;&nbsp;*&nbsp;".GetLangVar('namegeocoordnote')."</td>
</tr>";
...
<td colspan=100% align='center'>
	<table>
		<tr>
			<td><input type='submit' name='submitbutton' value=".GetLangVar('nameenviar')." class='bsubmit'></td>
</form>
<form action=localidade-form.php method='post'>	
<td><input type='submit' value=".GetLangVar('namereset')." class='breset'>
</td>
</tr>
</table></td>
</form>
