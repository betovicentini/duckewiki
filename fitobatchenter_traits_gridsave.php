...
$colvalid[] = '';
$colvalidtorf[] = 'false';
$qcol = "SELECT CONCAT(UPPER(Sobrenome),'  ',Prenome,' ', SegundoNome) AS coletor FROM `Pessoas` ORDER BY Sobrenome,Prenome,SegundoNome";
$rcol = mysql_query($qcol,$conn);
$coletores = array();
while ($rwcol = mysql_fetch_assoc($rcol)) {
	$coletores[] = $rwcol['coletor'];
}
$coltxt = implode(",",$coletores);
$selectboxes["TEMP_COLETOR"] = $coltxt;
$collist[] = $iddx;
$iddx++;
...
mysql_query(\"UPDATE `".$tbname."` SET `TEM_ETIQUETA`='{\$action->get_value('TEM_ETIQUETA')}'  WHERE `PlantaID`='{\$action->get_id()}'\");
mysql_query(\"UPDATE `".$tbname."` SET `TEMP_COLETOR`='{\$action->get_value('TEMP_COLETOR')}'  WHERE `PlantaID`='{\$action->get_id()}'\");
mysql_query(\"UPDATE `".$tbname."` SET `TEMP_NUMBER`='{\$action->get_value('TEMP_NUMBER')}'  WHERE `PlantaID`='{\$action->get_id()}'\");
\$val = \$action->get_value('TEMP_DATA_COLETA') ;
if (!empty(\$val) && \$val>0) {
...
echo "
  <form name='myform' action='fitobatchenter-traits-gridview.php' method='post'>";
