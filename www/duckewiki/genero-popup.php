...
$qu = "ALTER TABLE `Tax_Generos`  ADD `BibID` INT(10) NULL COMMENT 'BibtexID' AFTER `Morfotipo`";
@mysql_query($qu,$conn);
$qu = "ALTER TABLE `ChangeTax_Generos`  ADD `BibID` INT(10) NULL COMMENT 'BibtexID' AFTER `Morfotipo`";
@mysql_query($qu,$conn);

$qu = "ALTER TABLE `Tax_Generos`  ADD `MycobankID` INT(10) NULL AFTER `IpniID`";
@mysql_query($qu,$conn);
$qu = "ALTER TABLE `ChangeTax_Generos`  ADD `MycobankID` INT(10) NULL AFTER `IpniID`";
@mysql_query($qu,$conn);
...
if ($genusid>0 && $final!=1) {
	...
	$ttid = GetLangVar('nameeditar')."&nbsp;".mb_strtolower(GetLangVar('namegenus'))." &nbsp;<i>$spnome</i>";
} //if editing
else {
	$ttid = GetLangVar('namenovo')."&nbsp;".mb_strtolower(GetLangVar('namegenus'));
}
$erro=0;
if ($final==1) {
...
<tr class='tdsmallbold' ><td class='tdsmallnotes' align='center'>".GetLangVar('nameautor')."</td></tr>";
	...
	if ($erro==0) {
		...
		//get old values
		if ($genusid>0) {
			$qq = "SELECT FamiliaID,Genero,GeneroAutor,Basionym,BasionymAutor,PubRevista,PubVolume,PubAno,Sinonimos,Notas,Valid, IpniID,MycobankID FROM Tax_Generos WHERE GeneroID='".$genusid."'";
...
<br />";
		...
		else { //insert new genus
			$qq = "SELECT * FROM Tax_Generos WHERE LOWER(Genero) LIKE '".mb_strtolower($spnome)."' AND FamiliaID='".$famid."'";
			$qu = mysql_query($qq,$conn);
...
<thead>
  <tr><td >".$ttid."</td></tr>
  <tr class='subhead'><td >Checar os dados do gênero ".$spnome."</td></tr>
</thead>
<tbody>";
...
echo "
    <tr>
      ...
      <td align='center'>
        ...
        <td align='center'>
        <input type='submit' value='Checar Mycobank' class='borange' onclick=\"javascript:document.ipniform.ipnichecked.value=3\" />
      </td>
    </tr>
  </table>
  </td>
...
<tr bgcolor = '".$bgcolor."'>  
    ...
    <td>
      ...
      echo "
      <tr><td colspan=3>
      <input type='hidden' name='ipniid' value='".$ipniid."'  />IPNI No. ".$ipniid."&nbsp;<img style='cursor:pointer;' src='icons/mobot.png' height='18' onclick=\"javascript:small_window('http://www.tropicos.org/NameSearch.aspx?name=".$name."',800,600,'Tropicos');\" onmouseover=\"Tip('Ver registro do nome ".$name." em tropicos.org');\" /></td>
      </tr>";
      }
echo "  
      </table>
