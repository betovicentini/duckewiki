...
getaltitude(pltb.Altitude+0, pltb.GPSPointID,pltb.GazetteerID) as ALTITUDE";
		...
		if ($formvariables>0) {
			...
			TraitDefinicao, 
			tr.PathName 
			FROM Traits as tr JOIN FormulariosTraitsList ON FormulariosTraitsList.TraitID=tr.TraitID  WHERE FormulariosTraitsList.FormID=".$formvariables."  ORDER BY FormulariosTraitsList.Ordem";
		...
		if ($temcenso>0) {
			...
			foreach ($censos as $cen) {
				if ($cen>0) {
					...
					$cso = $cso2;
					$qu = "SELECT DISTINCT TraitID FROM Monitoramento JOIN FiltrosSpecs as fl ON Monitoramento.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtro." AND CensoID='".$cen."'";
		...
		$qq .= " JOIN FiltrosSpecs as fl ON pltb.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtro;


//A QUERY ESTA PRONTA, AGORA PRECISA GERAR O ARQUIVO
$qz = "SELECT * FROM Plantas  JOIN FiltrosSpecs as fl ON Plantas.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtro;
