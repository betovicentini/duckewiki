 ...
 * Copyright (c) 2005-2011, http://www.interpid.eu
 *
 ...
 * 
 */
class fpdf_multicelltag extends PDF_Label{
 ...
 * @var		boolean
 */
protected $wt_bDoubleTags = false;
...
protected function mt_Reset_Datas(){
	...
	public function SetStyle($tag, $family, $style, $size, $color)
	{
		...
		if (isset($this->TagStyle[$tag])) $this->wt_bDoubleTags = true;
	...
	protected function mt_MakeLine($w){
		...
		foreach ($aDataInfo as $key => $val){
	        ...
	        while ($i < $s_lenght){
	        	...
	        	if (!isset($fw[$tag]) || ($tag == "") || ($this->wt_bDoubleTags)){
	...
	public function MultiCellTag($w, $h, $pData, $border=0, $align='J', $fill=0, $pad_left=0, $pad_top=0, $pad_right=0, $pad_bottom=0, $pDataIsString = true){	
		...
		$aRecData = $this->mt_StringToLines($w, $pData);
		$iCounter = 9999; /*avoid infinite loop for any reasons*/
		
		$doBreak = false;
		
		do{ 
			$iLeftHeight = $this->h - $this->bMargin - $this->GetY() - $pad_top - $pad_bottom;
			$bAddNewPage = false;
			...
			$this->MultiCellTagSec($w, $h, $aSendData, $border, $align, $fill, $pad_left, $pad_top, $pad_right, $pad_bottom, false);
	 ...
	 * @return 	void
	 */	
	protected function MultiCellTagSec($w, $h, $pData, $border=0, $align='J', $fill=0, $pad_left=0, $pad_top=0, $pad_right=0, $pad_bottom=0, $pDataIsString = true){
        ...
        //if data is string
        if ($pDataIsString === true) $this->mt_DivideByTags($pData);
		...
		while(!$last_line){
			...
			$currentY = $this->getY();
			$restHeight = (int) $this->h - $this->tMargin - $this->bMargin;
			//$restHeight -= $h;
			
			
			//echo "$currentY - $restHeight<br>";
			
			//$restHeight -= 
			/**/
			
			/*
			if ($currentY > $restHeight){
				//die();
				$this->AddPage();
			}
			*/
	...
	*/
    ...
    protected function mt_DivideByTags($pStr, $return = false){
		...
		$sWork = new string_tags($this->wt_TagWidthMax);
	...
	protected function mt_PrintLine($w, $h, $aTxt, $align='J'){
		...
		if ($w_first != 0){
			$this->Cell($w_first, $h, "", self::DEBUG_CELL_BORDERS, 0, "L", 0);
		}
        ...
        while (list($key, $val) = each($aTxt)) {
			...
			$this->Cell($width, $h, $val['text'], self::DEBUG_CELL_BORDERS, 0, "C", 0, $val['href']);
