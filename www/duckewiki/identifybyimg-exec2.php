...
if ($filtro>0 && !isset($changed)) { 
		$qq = "(SELECT Especimenes.EspecimenID,NULL as PlantaID,TraitVariation,Traits.TraitName,Especimenes.DetID,CONCAT(Abreviacao,' ',Number) as ImgRef,NULL as ImgRef2,TraitTipo FROM Especimenes JOIN Identidade USING(DetID) JOIN Traits_variation ON Especimenes.EspecimenID=Traits_variation.EspecimenID JOIN Traits ON Traits.TraitID=Traits_variation.TraitID JOIN Pessoas ON Especimenes.ColetorID=Pessoas.PessoaID   JOIN FiltrosSpecs as fl ON Especimenes.EspecimenID=fl.EspecimenID WHERE TraitTipo LIKE '%Imagem%' AND fl.FiltroID=".$filtro.")";

		$qq = $qq." UNION (SELECT NULL as EspecimenID,Plantas.PlantaID,TraitVariation,Traits.TraitName,Plantas.DetID,PlantaTag as ImgRef1,InSituExSitu as ImgRef2,TraitTipo FROM Plantas JOIN Identidade USING(DetID) JOIN Traits_variation ON Plantas.PlantaID=Traits_variation.PlantaID JOIN Traits ON Traits.TraitID=Traits_variation.TraitID  JOIN FiltrosSpecs as fl ON Plantas.PlantaID=fl.PlantaID  WHERE TraitTipo LIKE '%Imagem' AND fl.FiltroID=".$filtro.")";
