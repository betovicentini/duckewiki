...
$qq = "SELECT CONCAT(ColetorID,'_',Number,Day,Mes,Ano) AS valor, GROUP_CONCAT(EspecimenID SEPARATOR ';') as ids,Abreviacao,Number,count(EspecimenID) as cnt FROM Especimenes JOIN Pessoas ON ColetorID=PessoaID ";
if ($acclevel!='admin') {
			$qq .= " WHERE Especimenes.AddedBy='".$uuid."'";
}
$qq .= "  GROUP BY CONCAT(ColetorID,'_',Number,Day,Mes,Ano) HAVING cnt>1 AND (valor IS NOT NULL)";
...
//<tr> <td align='left' ><input  type='button'  style=\"cursor:pointer;\"   value='Atualiza Vínculos'  onclick = \"javascript:passdupspecs('todeletespecs','".$passids."');\" />&nbsp;<img height=15 src=\"icons/icon_question.gif\" ";
