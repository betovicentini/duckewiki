...
$nf = array();
foreach ($specs as $vv) {
	$qq = "SELECT pl.PlantaID,sp.EspecimenID FROM Plantas as pl LEFT JOIN Especimenes AS sp ON sp.PlantaID=pl.PlantaID  JOIN FiltrosSpecs as fl ON pl.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtroid." AND pl.PlantaTag='".trim($vv)."' LIMIT 0,1";
...
<table class='success' align='center'>
  ...
  <form >
  <tr><td><input type=button value='".GetLangVar('namesalvar')." ".mb_strtolower(GetLangVar('namefiltro'))."' class='bsubmit' onclick = \"javascript:small_window('$myurl',350,280,'Filtro');\" /></td></tr>
  </form>
</table>";
