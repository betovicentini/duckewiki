...
if(isset($expeditoid) && $expeditoid=='criar') {
	header("location: expedito_ponto_form.php?ispopup=1");
	exit();
} 
if (empty($expeditoid)) {
	header("location: expedito_plantas_form.php?ispopup=1");
	exit();
}
...
echo "
<br />
<form name='coletaform' action='expedito_plantas_exec.php' method='post'>
...
if (empty($especimenids[$i])) {
		...
		$nsci  = "nomesciid_".$i;
 		echo "<td style='border: 0px'>"; 
					autosuggestfieldval3("search-name-simple.php","nomesci_".$i, $nomesci,"nomeres_".$i,"nomesciid_".$i,$$nsci,true,50); 
		echo "</td>";          
} else {
