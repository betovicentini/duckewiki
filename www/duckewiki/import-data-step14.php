...
//echopre($ppost);
//PEGA VARIAVEIS SUBMETIDAS PELO JAVASCRIPT EM import-data-step140.php
...
$ofim = $ntotal-$nrecstart;
$idx = 1;
while($nrecstart<$ntotal) {	
...
//cria um nome de filtro
$tbb = str_replace("temp_","",$tbname);
$filtronome = $tbb."_Importado_".$_SESSION['sessiondate'];

//salva a tabela final importada
$savetopathfile = "uploads/data_files/".$tbb.".sql";
backup_tables($tbname,$savetopathfile,$conn);
$tabelafinal = $savetopathfile;

//CRIA UM FORMULARIO DE VARIAVEIS  IMPORTADAS
if (count($arrayoffields['traitidsestatic'])>0 || count($arrayoffields['traitidsmoni'])>0) {
	$asvars = array_merge($arrayoffields['traitidsestatic'],$arrayoffields['traitidsmoni']);
	$trs = array_unique($asvars);
	$trstr =  implode(";",$trs);
	$fomrestatic = $tbb."_".$_SESSION['sessiondate'];
...
//SALVA UM FILTRO COM OS DADOS IMPORTADOS AMOSTRAS E/OU PLANTAS
...
if ($coletas==1 && $newfiltro>0) {
	$sql = "INSERT INTO FiltrosSpecs (FiltroID,PlantaID) (SELECT ".$newfiltro.",tb.`".$tbprefix."PlantaID` FROM `".$tbname."` as tb WHERE tb.`".$tbprefix."_CRMODIPLANTASIDS`=1)"; 
	//echo $sql;
	mysql_query($sql,$conn);
}

if (($coletas==2 || $coletas==3) && $newfiltro>0) {
	$sql = "INSERT INTO FiltrosSpecs (FiltroID,EspecimenID) (SELECT ".$newfiltro.",tb.`".$tbprefix."EspecimenID` FROM `".$tbname."` as tb WHERE tb.`".$tbprefix."_CRMODISPECSIDS`=1)"; 
	mysql_query($sql,$conn);
	//echo $sql;
}

$nspecs = count($arrayoffields['crmodi_especimenid']);
$nplantas = count($arrayoffields['crmodi_plantasids']);

//CRIA O OUTPUT DA IMPORTACAO
$output = "
<table cellpadding='5' class='myformtable' align='left' width='100%'>
<thead><tr><td >Atenção!</td></tr></thead>
<tbody>";
if ($arrayoffields['plins']>0) {
if ($bgi % 2 == 0){$bgcolor = $linecolor2 ;}  else{$bgcolor = $linecolor1 ;} $bgi++;
$output .= "
  <tr bgcolor = '".$bgcolor."'><td  >Foram inseridos ".$arrayoffields['plins']." valores para ".$nplantas." plantas marcadas</td></tr>";
}
if ($arrayoffields['plup']>0) {
if ($bgi % 2 == 0){$bgcolor = $linecolor2 ;}  else{$bgcolor = $linecolor1 ;} $bgi++;
$output .= "
  <tr bgcolor = '".$bgcolor."'><td  >Foram atualizados ".$arrayoffields['plup']." valores para ".$nplantas." plantas marcadas</td></tr>";
}
if ($arrayoffields['spins']>0) {
if ($bgi % 2 == 0){$bgcolor = $linecolor2 ;}  else{$bgcolor = $linecolor1 ;} $bgi++;
$output .= "
  <tr bgcolor = '".$bgcolor."'><td  >Foram inseridos ".$arrayoffields['spins']." valores para ".$nspecs." especimenes</td></tr>";
}
if ($arrayoffields['spup']>0) {
if ($bgi % 2 == 0){$bgcolor = $linecolor2 ;}  else{$bgcolor = $linecolor1 ;} $bgi++;
$output .= "
  <tr bgcolor = '".$bgcolor."'><td  >Foram atualizados ".$arrayoffields['spup']." valores para ".$nspecs." especimenes</td></tr>";
}
...
if ($bgi % 2 == 0){$bgcolor = $linecolor2 ;}  else{$bgcolor = $linecolor1 ;} $bgi++;
$output .= "
  <tr bgcolor = '".$bgcolor."'><td  >$ntr variáveis de usuário foram importadas e estão agrupadas no formulário <b>$fomrestatic</b></td></tr>";
}
if ($bgi % 2 == 0){$bgcolor = $linecolor2 ;}  else{$bgcolor = $linecolor1 ;} $bgi++;
$output .= "
<tr bgcolor = '".$bgcolor."'>
