...
<thead>
<tr><td colspan='100%'>".GetLangVar('namecadastrar')." ".mb_strtolower(GetLangVar('namevernacular'))."</td></tr>
</thead>
<tbody>
...
";
	...
	if ($submitted=='editando') {
		...
		$obs = $ww['Notes'];
		$ttxt =  GetLangVar('nameeditando')." ".mb_strtolower(GetLangVar('namecadastro'))." ".$nome;
	} 
	elseif ($submitted=='novo') {
		unset($_SESSION['editando']);
		$ttxt = GetLangVar('namenovo')." ".mb_strtolower(GetLangVar('namecadastro'));
	}
...
<tr bgcolor = '".$bgcolor."'>
  ...
  <td class='tdsmallbold' >
    <input type='text' name='lingua' size='20%' value='$lingua' /> ".mb_strtolower(GetLangVar('nameor'))."&nbsp;<select name='lingua2'>";
			if (!empty($lingua)) {
				echo "
