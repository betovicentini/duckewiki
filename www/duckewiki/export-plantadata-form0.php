...
elseif (($filtro+0)>0) {
$qz = "SELECT COUNT(*) as npl FROM Plantas AS pl  JOIN FiltrosSpecs as fl ON pl.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtro;
...
//já tem censos cadastrados
	$qz = "SELECT DISTINCT CensoID FROM Monitoramento  JOIN FiltrosSpecs as fl ON Monitoramento.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtro." AND CensoID>0";
	$rz = @mysql_query($qz,$conn);
      ...
      <select name='censos[]' multiple size='5'>";
			while ($rr = @mysql_fetch_assoc($rz)) {
				...
				$rwk = @mysql_fetch_assoc($rk);
								$qz = "SELECT COUNT(DISTINCT TraitID) as trs FROM Monitoramento JOIN FiltrosSpecs as fl ON Monitoramento.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtro." AND CensoID='".$rr['CensoID']."'";
				$rzk = @mysql_query($qz,$conn);
				$rwzk = @mysql_fetch_assoc($rzk);
				$trs = $rwzk['trs'];
				$qz = "SELECT COUNT(DISTINCT PlantaID) as pls FROM Monitoramento JOIN FiltrosSpecs as fl ON Monitoramento.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtro." AND CensoID='".$rr['CensoID']."'";
				$rzk = @mysql_query($qz,$conn);
				$rwzk = @mysql_fetch_assoc($rzk);
				$pls = $rwzk['pls'];
				echo "
