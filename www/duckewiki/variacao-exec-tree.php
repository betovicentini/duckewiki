...
$newimagefile = array();
if ($option1=='2') {
	unset($arval['MAX_FILE_SIZE'],  $arval['formid' ],  $arval['option1'],  $arval['plantaid'], $arval['final']);
	$result = array();
	...
	foreach ($arraryofvalue as $key => $value) {
		...
		if ($varorunit!='plantaid' && $varorunit!='traitimgautor' && $varorunit!='imagid' && $varorunit!= 'traitimgold' && $varorunit!='imgtodel' && $varorunit!='traitimgautortxt') {
...
if (count($newimagefile)>0) {
		...
		$returnto = $serv."/".$zz[1]."/variacao-exec-tree.php";
