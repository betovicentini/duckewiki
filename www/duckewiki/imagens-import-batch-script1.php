...
@mysql_query($qq,$conn);
$qq = "ALTER TABLE Imagens ADD COLUMN TraitID INT(10) DEFAULT 0";
@mysql_query($qq,$conn);


$qq = "ALTER TABLE Imagens ADD COLUMN UnLinked INT(10) DEFAULT 0 AFTER HabitatPhoto";
@mysql_query($qq,$conn);

$qq = "ALTER TABLE `ChangeImagens` ADD `TraitID` INT NULL DEFAULT '0' AFTER `AddedDate`";
@mysql_query($qq,$conn);

$qq = "ALTER TABLE `ChangeImagens` ADD `UnLinked` INT NULL DEFAULT '0' AFTER `HabitatPhoto`";
@mysql_query($qq,$conn);
...
'GPSPointID' => $gpsptid,
'Autores' => $addcolvalue,
'TraitID' => $ppost['traitid']+0,
'UnLinked' => $ppost['linkposterior']+0);
...
if ($counter_nolink>0) {
	$nao_importou = $nimg-$counter_nolink;
