...
$getRecord_sql = "SELECT tr.TraitName as nome,tr.TraitID,tr.PathName,tr.TraitTipo FROM Traits as tr WHERE (tr.TraitTipo LIKE '%Categoria%') AND (LOWER(tr.TraitName) LIKE '%".$searchq."%' OR getparentname(tr.ParentID) LIKE '%".$searchq."%')  ORDER BY tr.TraitName,getparentname(tr.ParentID)";
...
while ($row = mysql_fetch_array($getRecord)) {
	...
	$tgn = $row['nome']."  (".$txt.")";
	echo "
  <li style='background-color: #99CCFF;' ><a href=\"javascript:substitui('".$tgn."','".$idtag."','".$idres."', '".$row['TraitID']."', '".$nomeid."');\">".$tgn."</a></li>";
}
echo '
</ul>';
