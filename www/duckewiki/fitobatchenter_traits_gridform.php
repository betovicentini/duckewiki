...
$tbname = 'fitobatchenter_'.$sampletype.'_'.$filtroid.'_'.$formid;

if (empty($sampletype)) {
	$sampletype = 'especimenes';
}
if (empty($filtroid)) {
		header("location: fitobatchenter_traits_form.php");
}
...
"<script>
    function CheckProgress() {
      ...
      $.get('fitobatchenter_traits_gridprogress.php', { t: time, tbname: '".$tbname."'}, function (data) {
        ...
        if (progress < 100) {
          ...
          window.location = 'fitobatchenter_traits_gridsave.php?tbname=".$tbname."&formid=".$formid."&sampletype=".$sampletype."&ispopup=1';
    	}
      });
	}
    ...
    $.ajax(
            {
                type: 'GET',
                url: 'fitobatchenter_traits_gridscript.php',
                data: '".$varstxt."',
                async: true,
                success:
                    function (data) {
