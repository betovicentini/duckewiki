...
$qu = "ALTER TABLE `Tax_InfraEspecies`  ADD `MycobankID` INT(10) NULL AFTER `IpniID`";
@mysql_query($qu,$conn);
$qu = "ALTER TABLE `ChangeTax_InfraEspecies`  ADD `MycobankID` INT(10) NULL AFTER `IpniID`";
@mysql_query($qu,$conn);
...
if ($infraspid>0 && $final!=1) {
	...
	$ttid = GetLangVar('nameeditar')."&nbsp;".mb_strtolower(GetLangVar('nameinfraspecies'));
} //if editing
else {
	if ($speciesid>0) {
	...
	$ttid = GetLangVar('namenova')."&nbsp;".mb_strtolower(GetLangVar('nameinfraspecies'))."&nbsp;de <i>$nn</i>";
}
...
<br />";
	...
	if ($erro==0) {
		...
		if ($infraspid>0) {
			$qq = "SELECT EspecieID,InfraEspecie,InfraEspecieAutor,InfraEspecieNivel,BibID, Basionym,BasionymAutor,PubRevista,PubVolume,PubAno,Sinonimos,GeoDistribution,Notas,Valid,IpniID,MycobankID FROM Tax_InfraEspecies WHERE InfraEspecieID=".$infraspid;
...
<br />";
			...
			$qq = "SELECT * FROM Tax_InfraEspecies WHERE LOWER(InfraEspecie) LIKE '".mb_strtolower($spnome)."' AND EspecieID='".$speciesid."'";
			$qu = mysql_query($qq,$conn);
...
<form action='infraespecie-popup.php' method='post'>
  ...
  <tr>
    <td>
      <input type='radio' name='morfoorpubsp' value='1' onchange=\"this.form.submit();\" />&nbsp;Nome infraespecífico publicado
    </td>
  </tr>
  <tr align='left'>
    <td>
      <input type='radio' name='morfoorpubsp'  value='2' onchange='this.form.submit();' />&nbsp;Morfotipo infraespecífico
    </td>
  </tr>
  </tbody>
  </table>
...
//if ($infraspid>0) {
	//$qu = "SELECT InfraEspecie,Especie,GeneroID,Genero, EspecieID FROM Tax_InfraEspecies JOIN Tax_Especies USING(EspecieID) JOIN Tax_Generos USING(GeneroID) WHERE InfraEspecieID='".$infraspid."'";
...
<thead>
  <tr><td >".$ttid."</td></tr>
  <tr class='subhead'><td >Checar dados da infra-espécie ".$genero." ". $especie." ".$spnome."?</td></tr>
</thead>
<tbody>";
if ($bgi % 2 == 0) {$bgcolor = $linecolor2 ;} else { $bgcolor = $linecolor1 ;} $bgi++;
echo "
...
<br />";
		...
		if ($ipnichecked==3 && !empty($spnome)) {
			$resultado = getmycobankdata($genero,$especie,"",$spnome);
			//echopre($resultado);
