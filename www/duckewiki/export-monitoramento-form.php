...
$title = 'Exportar dados de monitoramento de plantas';
$body = '';
...
echo "
<form method='post' name='finalform' action='export-monitoramento-form.php'>
...
elseif (($filtro+0)>0) {
$qz = "SELECT COUNT(*) as npl FROM Plantas AS pl  JOIN FiltrosSpecs as fl ON pl.PlantaID=fl.PlantaID WHERE FiltroID=".$filtro;
...
//já tem censos cadastrados
	$qz = "SELECT DISTINCT CensoID FROM Monitoramento  JOIN FiltrosSpecs as fl ON Monitoramento.PlantaID=fl.PlantaID WHERE FiltroID=".$filtro." AND CensoID>0";
	$rz = @mysql_query($qz,$conn);
	$ncensos = @mysql_numrows($rz);
	if ($ncensos>0) {
		if ($bgi % 2 == 0){$bgcolor = $linecolor2 ;}  else {$bgcolor = $linecolor1 ;} $bgi++;
		echo "
<form method='post' name='finalform' action='export-monitoramento-exec.php'>
<input type='hidden' name='ispopup' value='".$ispopup."' /> 
      ...
      <select name='censos[]' multiple size='5'>";
			while ($rr = @mysql_fetch_assoc($rz)) {
				...
				$rwk = @mysql_fetch_assoc($rk);
								$qz = "SELECT COUNT(DISTINCT TraitID) as trs FROM Monitoramento  JOIN FiltrosSpecs as fl ON Monitoramento.PlantaID=fl.PlantaID WHERE FiltroID=".$filtro." AND CensoID='".$rr['CensoID']."'";
				$rzk = @mysql_query($qz,$conn);
				$rwzk = @mysql_fetch_assoc($rzk);
				$trs = $rwzk['trs'];
				$qz = "SELECT COUNT(DISTINCT PlantaID) as pls FROM Monitoramento   JOIN FiltrosSpecs as fl ON Monitoramento.PlantaID=fl.PlantaID WHERE FiltroID=".$filtro." AND CensoID='".$rr['CensoID']."'";
				$rzk = @mysql_query($qz,$conn);
				$rwzk = @mysql_fetch_assoc($rzk);
				$pls = $rwzk['pls'];
				echo "
...
<table>
<tr>
  <td class='tdsmallbold'>".GetLangVar('nameformulario')." ".mb_strtolower(GetLangVar('namevariaveis')." estáticas");
echo "&nbsp;<img height='14' src=\"icons/icon_question.gif\" ";
...
<table>
<tr>
  <td class='tdsmallbold'>".GetLangVar('nameformulario')." ".mb_strtolower(GetLangVar('namevariaveis')." monitoramento");
echo "&nbsp;<img height='14' src=\"icons/icon_question.gif\" ";
$help = strip_tags(GetLangVar('exportformulariovariaveis_monitorhelp'));
echo " onclick=\"javascript:alert('$help');\" /></td>
  <td>
    <table>
      <tr>
        <td class='tdformnotes'>
          <select name='formvariables_monitor' >";
		if (!empty($formid)) {
			$qq = "SELECT * FROM Formularios WHERE FormID='$formid'";
			$rr = mysql_query($qq,$conn);
			$row= mysql_fetch_assoc($rr);
	...
	echo "
        ...
        <td class='tdformnotes' ><input type='checkbox' value='1' name='meanvalues_monitor' />".GetLangVar('usemeanvalues')."&nbsp;<img height=14 src=\"icons/icon_question.gif\" ";
	$help = strip_tags(GetLangVar('usemeanvalues_help'));
	echo " onclick=\"javascript:alert('$help');\" /></td>
      </tr>
    </table>
...
<tr bgcolor = '".$bgcolor."'>
  <td >
    <table>
      <tr>
        <td class='tdsmallbold'>".GetLangVar('nameformulario')." ".mb_strtolower(GetLangVar('namevariaveis')." ".GetLangVar('namehabitat'));
		echo "&nbsp;<img height='14' src=\"icons/icon_question.gif\" ";
