...
function myUpdate($action){
    ...
    $idsp = $action->get_id();    
    $ru = mysql_query("SELECT Marcado FROM `checklist_allUserLists` WHERE TempID=".$idsp."  AND UserID='70'");  
    $nru = mysql_numrows($ru);
    if ($nru!=$status) {
       if ($status==1) {    
     $qinn = "INSERT INTO  `checklist_allUserLists` (`Marcado`,`TempID`,`UserID`) VALUES ('1' ,'".$idsp."','70')";  
     }  else {    
     $qinn = "DELETE FROM  `checklist_allUserLists`  WHERE TempID='".$idsp."' AND UserID='70'";  
     }  
     $ru = mysql_query($qinn);
   }          
   $action->success();
...
function custom_format_list($data){
    ...
    $recid = $data->get_id();    
    $ru = mysql_query("SELECT Marcado FROM `checklist_allUserLists` WHERE TempID=".$recid."  AND UserID='70'");    
...
$plnumb = $data->get_value("PLANTAS");
 if ($plnumb>0) {
$imagen= "<sup>  ".$data->get_value("PLANTAS")."</sup><img style='cursor:pointer;' src='icons/tree-icon.png' height='20'  onclick=\"javascript:small_window('http://localhost/duckewiki/www/duckewiki/checkllist_plantas_save.php?tbname=checklist_pllist&famid=".$famid."&genid=".$genid."&specid=".$specid."&infspecid=".$infspecid."',950,500,'Plantas');\"  onmouseover=\"Tip('Visualizar plantas desse taxon');\" />";
} else {
  $imagen = " ";
}
...
if (($data->get_value("PLOTS"))>0) {
	...
	$imagen2 = "<img style='cursor:pointer;' src='icons/diversity.png' height='18' onclick=\"javascript:small_window('http://localhost/duckewiki/www/duckewiki/taxa-form.php?final=1&nomesciid=nomesciid&famid=".$famid."&genusid=".$genid."&speciesid=".$specid."&infraspid=".$infspecid."&ispopup=1',700,500,'Editando nome');\" onmouseover=\"Tip('Editando a taxonomia de ".$nameedit."');\" alt='' />";
	if ($specid>0 || $infspecid>0) {
	$imagen3 = "<img style='cursor:pointer;' src='icons/changename.png' height='18' onclick=\"javascript:small_window('http://localhost/duckewiki/www/duckewiki/identifybyname_all.php?nomesearch=".$nameedit."&speciesid=".$specid."&infraspid=".$infspecid."&tempid=".$recid."&tbname=checklist_all',700,500,'Substituindo o nome ".$nameedit." por outro');\" onmouseover=\"Tip('Substituindo o nome ".$nameedit." por outro');\" alt='' />";
	}   $imgg33 ="<img style='cursor:pointer;' src='icons/nota-icon.png' height='16' onclick=\"javascript:small_window('http://localhost/duckewiki/www/duckewiki/traits_coletortaxa.php?nomesciid=".$nomesciid."&taxanome=$nameedit',800,800,'Editando notas');\"  onmouseover=\"Tip('Edita notas do taxon # ".$tropicos."');\" >";
