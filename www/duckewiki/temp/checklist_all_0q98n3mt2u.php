...
function myUpdate($action){
    ...
    $idsp = $action->get_id();    
    $ru = mysql_query("SELECT Marcado FROM `checklist_allUserLists` WHERE TempID=".$idsp."  AND SessionID='0q98n3mt2u'");  
    $nru = mysql_numrows($ru);
    if ($nru!=$status) {
       if ($status==1) {  
     $qinn = "INSERT INTO  `checklist_allUserLists` (`Marcado`,`TempID`,`SessionID`) VALUES ('1' ,'".$idsp."','0q98n3mt2u')";  
     }  else {  
     $qinn = "DELETE FROM  `checklist_allUserLists`  WHERE TempID='".$idsp."' AND SessionID='0q98n3mt2u'";  
     }  
     $ru = mysql_query($qinn);
   }          
   $action->success();
...
function custom_format_list($data){
    ...
    $recid = $data->get_id();    
    $ru = mysql_query("SELECT Marcado FROM `checklist_allUserLists` WHERE TempID=".$recid."  AND SessionID='0q98n3mt2u'");    
