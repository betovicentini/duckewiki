...
function myUpdate($action){
    ...
    $idsp = $action->get_id();    
    $ru = mysql_query("SELECT Marcado FROM `checklist_allUserLists` WHERE TempID=".$idsp."  AND SessionID='e8v584ju4g'");  
    $nru = mysql_numrows($ru);
    if ($nru!=$status) {
       if ($status==1) {  
     $qinn = "INSERT INTO  `checklist_allUserLists` (`Marcado`,`TempID`,`SessionID`) VALUES ('1' ,'".$idsp."','e8v584ju4g')";  
     }  else {  
     $qinn = "DELETE FROM  `checklist_allUserLists`  WHERE TempID='".$idsp."' AND SessionID='e8v584ju4g'";  
     }  
     $ru = mysql_query($qinn);
   }          
   $action->success();
...
function custom_format_list($data){
    ...
    $recid = $data->get_id();    
    $ru = mysql_query("SELECT Marcado FROM `checklist_allUserLists` WHERE TempID=".$recid."  AND SessionID='e8v584ju4g'");    
