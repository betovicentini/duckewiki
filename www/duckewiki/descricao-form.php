...
"<script>
    function CheckProgress() {
		...
		pgxhttp.onreadystatechange = function() {
			if (pgxhttp.readyState == 4 && pgxhttp.status == 200) {
				...
				if (progress<100) {
					document.getElementById(\"oprg\").innerHTML = 'Processando ' + progress + '%';	         	         
  ...
  function makedescription() {	
  		var otxt = document.getElementById('taxon').options;
  		var lx = document.getElementById('lang').selectedIndex;
		var ostaxids = [];
		var idx = 0;
		for(v=0;v<otxt.length;v++) {
   		var sel = otxt[v].selected;
   		if (sel) { ostaxids[idx] = otxt[v].value; idx++;}
		}
 		ostaxids = ostaxids.join(';');
		...
		var frm = document.getElementById('frm');
 		...
 		if(frmid>0 & idx>0) {
			...
			var url = 'descricao-make-all.php?pgfilename=".$pgfilename."&falta='+faltaval+'&lang='+lang+'&formid='+frmid+'&oprojetoid=".$projetoid."&ostaxids='+ostaxids;
		//alert(url);
...
if ($projetoid>0) {
	...
	IF(tbb.InfraEspecieID>0,tbb.InfraEspecieID,NULL) AS InfraEspecieID FROM (".$sqlw." UNION ".$sqlw2.") as tbb WHERE tbb.EspecieID>0 ORDER BY tbb.Familia,tbb.Genero,tbb.Especie,tbb.InfraEspecie"; 
	
}
echo "
<form action='descricao-form.php'  method='post' >
...
<div >
<select  id='taxon' style=\"color:#000000; font-size: 1em; font-weight:bold; padding: 4px; cursor:pointer;\" name='taxonid' multiple size=10 >
";
$rsql = mysql_query($sqlinfr,$conn);
$resultado = '';
while($rsqlw = mysql_fetch_assoc($rsql)) {
		echo "
  <option value='".$rsqlw['EspecieID']."-".$rsqlw['InfraEspecieID']."'>".$rsqlw['Genero']." ".$rsqlw['Especie']." ".$rsqlw['InfraEspecie']."[".$rsqlw['Familia']."]</option>";
	}	
echo "
</select>";
	echo "
...
<div id=\"oprg\"></div>
<div id=\"modelo\">";
$ofname = "temp/monografiaspp_".$projetoid.".php";
if (file_exists($ofname)) {
echo "<a href='".$ofname."'>Monografia.doc [Gerado em ".date ("F d Y H:i:s.", filemtime($ofname))."]</a>";
}
echo "</div>
</div>
</div>
