...
<br />
  <table class='erro' align='center' cellpadding=\"5\" >
    <tr><td>Precisa dar um nome ao formulário!</td></tr>
		...
		$curdate = date("Y-m-d");
		$formularionome = trim($formularionome)."_".$curdate;
		...
		if ($nrss>0)  {
			$formularionome = $formularionome."_A";
		...
		if ($formid) {
			...
			foreach ($traitarr as $value) {
					...
					$qz = "INSERT INTO FormulariosTraitsList (FormID,TraitID,Ordem) SELECT ".$formid.",".$vv.",".$n;
