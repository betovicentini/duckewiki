...
if (($final>0 && !empty($nomesciid)) || ($final==2))  {
	...
	if ($final==1 && !empty($nomesciid)) { 
		...
		if ($final==4) { $filename = 'traits_coletortaxa.php';}
	}
	if (!empty($filename)) {
	echo "
<br/>
...
<input type='hidden' name='ispopup' value='".$ispopup."' />
<table align='left' class='myformtable' cellspacing='0' cellpadding='5'>
<thead>
<tr><td >".GetLangVar('namecadastrar')." ".mb_strtolower(GetLangVar('nameor')." ".GetLangVar('nameeditar')." ".GetLangVar('nametaxa'))."</td></tr>
</thead>
<tbody>
";
