...
echo "<tr><td colspan=2>".GetLangVar('namecadastrar')." ".mb_strtolower(GetLangVar('namevariavel'))." ".mb_strtolower($tip)."</td></tr>
</thead>
<tbody>";
...
if (!isset($traittipo)) {
	...
	if ($bgi % 2 == 0){$bgcolor = $linecolor2 ;}  else{$bgcolor = $linecolor1 ;} $bgi++;
	echo "<tr bgcolor = $bgcolor>	
	<td class='tdsmallbold'>".GetLangVar('nameselect')." ".mb_strtolower(GetLangVar('nametipo'))."</td>
...
UNION (SELECT DISTINCT TraitUnit as unit FROM Traits WHERE TraitUnit IS NOT NULL AND TraitUnit!='') ) as mytab ORDER BY unit";
						...
						&nbsp;&nbsp;".mb_strtolower(GetLangVar('namenova'))."&nbsp;&nbsp;</td>
				</tr></table>
			</td></tr>";
}
