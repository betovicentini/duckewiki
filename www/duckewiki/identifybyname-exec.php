...
if (!empty($nomesciid) && $nomesciid!='nomesciid' && $filtro>0) {
		...
		$qq = "SELECT * FROM Especimenes JOIN Identidade USING(DetID) JOIN FiltrosSpecs as fl ON Especimenes.EspecimenID=fl.EspecimenID WHERE ".$taxcol." AND fl.FiltroID=".$filtro;
		$res = mysql_query($qq,$conn);		
		$numspecs = mysql_numrows($res);

		$qq = "SELECT * FROM Plantas JOIN Identidade USING(DetID) JOIN FiltrosSpecs as fl ON Plantas.PlantaID=fl.PlantaID WHERE ".$taxcol." AND fl.FiltroID=".$filtro;
		$res = mysql_query($qq,$conn);		
...
if ($newdetid>0) {
	...
	mysql_query($qq,$conn);
	if ($numspecs>0) {
		$qq = "CREATE TABLE temp_detall_".$chgby." (SELECT Especimenes.*,".$chgby." as ChangedBy, '".$udate."' as ChangedDate FROM Especimenes JOIN Identidade USING(DetID)  JOIN FiltrosSpecs as fl ON Especimenes.EspecimenID=fl.EspecimenID WHERE ".$taxcol." fl.FiltroID=".$filtro.")";
		mysql_query($qq,$conn);
	...
	if ($numplantas>0) {
		$qq = "CREATE TABLE temp_detall_".$chgby." (SELECT Plantas.*,".$chgby." as ChangedBy, '".$udate."' as ChangedDate FROM Plantas JOIN Identidade USING(DetID) JOIN FiltrosSpecs as fl ON Plantas.PlantaID=fl.PlantaID WHERE ".$taxcol." AND fl.FiltroID=".$filtro.")";
		mysql_query($qq,$conn);
