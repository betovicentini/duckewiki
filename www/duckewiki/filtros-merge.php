...
";
	...
	if ($nf>1) {
		...
		if ($newfiltro>0) {
			$sql = "DELETE FROM FiltrosSpecs WHERE FiltroID=".$ftokeep;
			$rr = mysql_query($sql,$conn);
			foreach($specsarr as $spid) {
				$sql = "INSERT INTO FiltrosSpecs (FiltroID,EspecimenID) VALUES (".$ftokeep.",".$spid.")";
				mysql_query($sql,$conn);
			}
			foreach($plantasarr as $spid) {
				$sql = "INSERT INTO FiltrosSpecs (FiltroID,PlantaID) VALUES (".$ftokeep.",".$spid.")";
				mysql_query($sql,$conn);
			}
