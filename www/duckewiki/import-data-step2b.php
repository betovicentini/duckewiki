...
";
	...
	while ($rw = mysql_fetch_assoc($rq)) {
		...
		if (!$kkv && $fieldname!='ImportID' && substr($fieldname,0,$npr)!=$tbprefix) {
				$qf = "SELECT * FROM Import_Fields WHERE LOWER(NamesToMatch) LIKE '".mb_strtolower($fieldname)."%' OR LOWER(NamesToMatch) LIKE '%;".mb_strtolower($fieldname).";%' OR LOWER(NamesToMatch) LIKE '%;".mb_strtolower($fieldname)."'";
...
echo "
				...
				while ($rwqf = mysql_fetch_assoc($rqf)) {
					...
					if ($ipid==$rwqf['id']) {
						...
						if ($brh=="ESTA_VAR") {$ch='selected';} else {$ch ='';}
					}
					if ($pa!=$cl) {
						echo "
