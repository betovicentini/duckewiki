...
class PHPExcel_Calculation_Functions {
	...
	public static function INTERCEPT($yValues,$xValues) {
		if ((!self::_checkTrendArray($yValues)) || (!self::_checkTrendArray($xValues))) {
			return self::$_errorCodes['value'];
		}
...
//			$ta = 2 / pi();
	...
	public static function VERSION() {
		return 'PHPExcel 1.7.1, 2009-11-02';
	}	//	function VERSION()
...
//			$fRet = sqrt(0.636619772 / $fNum) * sin($fNum - 2.356194491);
	...
	public static function COUPNUM($settlement, $maturity, $frequency, $basis=0) {
		...
		$dsm = self::YEARFRAC($settlement, $maturity, $basis) * 365;
	...
	public static function MATCH($lookup_value, $lookup_array, $match_type=1) {
		...
		for ($i=0;$i<sizeof($lookup_array);++$i){
			...
			if (is_string($lookup_array[$i]))
				$lookup_array[$i] = mb_strtolower($lookup_array[$i]);
		}
	...
	private static function _vlookupSort($a,$b) {
		...
		if (strtolower($a[$firstColumn]) == mb_strtolower($b[$firstColumn])) {
			return 0;
		}
		return (strtolower($a[$firstColumn]) < mb_strtolower($b[$firstColumn])) ? -1 : 1;
	...
	*/
	public static function VLOOKUP($lookup_value, $lookup_array, $index_number, $not_exact_match=true) {
		...
		foreach($lookup_array as $rowKey => $rowData) {
			if (strtolower($rowData[$firstColumn]) > mb_strtolower($lookup_value)) {
				break;
			}
			$rowNumber = $rowKey;
