dhtmlXGridObject.prototype.toPDF=function(url,mode,header,footer,rows){
	...
	var win = window.open('exportGRIDasCSV.php', name='_blank',specs='scrollBars=yes,resizable=yes,toolbar=no,menubar=no,location=no,directories=no,width=400,height=200');
	win.document.open();
	win.document.write("<html><body>");
	win.document.write('<table align="center" cellpadding="7"><tr><td><b>Aguarde!</b></td></tr></table>');
	win.document.write('<form id="myform" method="post" action="'+url+'" accept-charset="utf-8" enctype="multipart/form-data"><input type="hidden" name="grid_xml" id="grid_xml"/> </form>');
	// as long dash in post converts to some special symbols we need to convert it to simple dash by replacing using unicode \u2013
	win.document.getElementById("grid_xml").value = xml_top(mode).replace("\u2013", "-") + xml_body() + xml_end();
