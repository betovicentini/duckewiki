...
//$tbname = 'batchenter_especimenes_386_89';
//$filtroid = 386;
//$formid = 89;
...
if ($sampletype=='especimenes') {
	...
	checknir(pltb.EspecimenID,pltb.PlantaID) AS NIR,";
} else {
	...
	checknir(0,pltb.PlantaID) AS NIR,
";
}
...
if ($sampletype=='especimenes') {
	$qq .= " FROM Especimenes as pltb LEFT JOIN Pessoas as colpessoa ON pltb.ColetorID=colpessoa.PessoaID";
	$qwhere = " JOIN FiltrosSpecs as fl ON fl.EspecimenID=pltb.EspecimenID WHERE fl.FiltroID=".$filtroid;
} else {
	$qq .= " FROM Plantas as pltb";
	$qwhere = " JOIN FiltrosSpecs as fl ON fl.PlantaID=pltb.PlantaID WHERE fl.FiltroID=".$filtroid;
}
$qq .= "
...
if ($step>$nsteps) {
	...
	if ($sampletype=='especimenes') {
		...
		$qq = "ALTER TABLE `".$tbname."`  ADD `TEMP_DATA_COLETA` DATE DEFAULT NULL AFTER `EXISTE`";
		mysql_query($qq,$conn);
