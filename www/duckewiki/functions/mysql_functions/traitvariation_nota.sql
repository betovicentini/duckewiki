CREATE FUNCTION traitvariation_nota(formid int(10), linkid INT(10)) RETURNS text CHARSET utf8
BEGIN
...
DECLARE cur1 CURSOR FOR SELECT Traits.TraitTipo,Traits.TraitName,Traits.PathName,TraitVariation,Traits_variation.TraitUnit FROM Traits_variation JOIN Traits USING(TraitID) JOIN FormulariosTraitsList ON FormulariosTraitsList.TraitID=Traits.TraitID WHERE Traits_variation.EspecimenID=linkid AND FormulariosTraitsList.FormID=formid  ORDER BY FormulariosTraitsList.Ordem;
DECLARE cur2 CURSOR FOR SELECT TraitTipo,TraitName,Traits.PathName,TraitVariation,Traits_variation.TraitUnit FROM Traits_variation JOIN Traits USING(TraitID) WHERE Traits_variation.EspecimenID=linkid ORDER BY Traits.PathName;
