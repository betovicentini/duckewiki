...
END IF;
IF padrao=1 THEN
	SELECT pl.PlantaID, count(*) into resultado, nn  FROM Plantas as pl JOIN FiltrosSpecs as fl ON fl.PlantaID=pl.PlantaID WHERE pl.PlantaTag=plantan AND fl.FiltroID=filtro;
END IF;
IF nn=1 THEN
	RETURN resultado;
ELSE
