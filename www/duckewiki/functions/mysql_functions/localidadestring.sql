...
IF (gpsptid>0 AND latt=9999 AND longg=9999) THEN
	SELECT gps.Latitude,gps.Longitude, ROUND(gps.Altitude+0,0) INTO latt,longg,altt FROM GPS_DATA as gps WHERE PointID=gpsptid;
	SET coordref = 'GPS  ';
END IF;
IF (gazid>0 AND latt=9999 AND longg=9999) THEN
			...
			DO
				SELECT (gs.Latitude+0),(gs.Longitude+0), ROUND(gs.Altitude+0,0), ParentID INTO latt,longg,altt,gazparid FROM Gazetteer as gs WHERE gs.GazetteerID=gazrunid;
