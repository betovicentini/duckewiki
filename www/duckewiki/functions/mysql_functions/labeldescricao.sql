...
DECLARE statevar2txt CHAR(100) DEFAULT "";
...
IF ttipo='Variavel|Quantitativo' THEN
	...
	SET valmax = '';
	WHILE ncatstep <= ncat DO
		SELECT (SUBSTRING_INDEX(SUBSTRING_INDEX(tvariation,';',ncatstep),';',-1)) INTO statevar2txt;
		SELECT REPLACE(statevar2txt,",",".") INTO statevar2txt;
		SET statevar2= statevar2txt+0;
		IF (ncatstep=1) THEN
