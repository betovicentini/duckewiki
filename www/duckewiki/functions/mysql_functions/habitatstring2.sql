CREATE FUNCTION habitatstring2(habid int(10), formid int(10), printn BOOLEAN, meanonly BOOLEAN) RETURNS text CHARSET utf8
BEGIN
...
DECLARE cur1 CURSOR FOR SELECT TraitTipo,TraitName,Traits.PathName,HabitatVariation,Habitat_Variation.TraitUnit FROM Habitat_Variation JOIN Traits USING(TraitID) JOIN FormulariosTraitsList ON FormulariosTraitsList.TraitID=Traits.TraitID  WHERE Habitat_Variation.HabitatID=habid AND FormulariosTraitsList.FormID=formid  ORDER BY FormulariosTraitsList.Ordem;
...
SET ttbase=ttbase2;
IF contador=0 THEN
SET descricao = ttbase2;
ELSE
SET descricao = CONCAT(descricao,'. <b>',ttbase2,'</b>');
END IF;
...
END LOOP loop1;
IF (habid>0) THEN
SELECT hab.PathName,hab.HabitatTipo,hab.EspeciesIds INTO habclass,habtipo,txlist FROM Habitat as hab WHERE HabitatID=habid;
IF (habtipo='Class') THEN
SET descricao = habclass;
ELSE
...
SET descricao = CONCAT(habclass,'. ',descricao,'.',txlist2);
