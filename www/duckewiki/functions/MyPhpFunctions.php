<?php
function omenudeicons($quais, $vertical=FALSE, $position='right', $iconwidth='35', $iconheight='35' ,$meuprojetoid) { 
if ($vertical) {
	$separador= '<br />';
	$hgt = "width=\"".$iconwidth."\"";
} else {
...
if ($position=='right') {
	...
	$poss = "position: relative; align: left; padding-top: 10px;";
	//$poss = 'float: left;';
}
...
if ($_SESSION['userid']>0) {
	$rez = mysql_query("SELECT ProjetoID FROM Projetos WHERE AddedBy=".$_SESSION['userid']);
	$nrez = mysql_numrows($rez);
	if ($nrez==1) {
		$rezw = mysql_fetch_assoc($res);
		$meuprojetoid = $rezw['ProjetoID'];
	}
...
'meuprojetos' => "<a style=\"padding: 10px;\" href=\"#\" class=\"menuicons_azulescuro\" onclick = \"javascript:small_window('projeto-form.php',1100,700,'Meus Projetos');\" >Meus Projetos</a>"
);
//"<img src=\"icons/science.png\" ".$hgt." style=\"".$stilo."\" onmouseover=\"Tip('Dados do seu projeto');\" onclick = \"javascript:small_window('projeto-form.php?submitted=editando&projetoid=".$meuprojetoid."&ispopup=0',1100,700,'Dados do seu projeto');\" />",
...
$it = strip_tags($iintrotext);
	if (!empty($it)) {
	echo "<div 
style='padding-top: 30px; padding-bottom: 0px; font-family:\"Verdana\", Arial, sans-serif;  font-size: 1.1em; font-color: #800000;'>".$_SESSION['introtext']."</div>";
	}
}
...
function formulariomodelo($specid,$plantaid,$formid) {
$sql = "SELECT tr.TraitID, tr.TraitTipo, prefixo , prefixoprevio,ucfirstvalue, sufixo , formato , unitformat , unitinclude ,namostral ,categpontuacao, categtxt FROM FormulariosTraitsList  as ff JOIN Traits as tr USING(TraitID) WHERE FormID=".$formid." ORDER BY Ordem";
$res = mysql_query($sql);
$descricao = "";
$oprevioprefixo = "";
$valorprevio = NULL;
while($row = mysql_fetch_assoc($res)) {
	$newunit = $row['unitformat'];
	$comunidade = $row['unitinclude'];
	$otipo  = $row['TraitTipo'];
	$ucfirstvalue = $row['ucfirstvalue'];
	$valor = "";
	if ($otipo=='Variavel|Quantitativo' || $otipo=='Variavel|Categoria' || $otipo=='Variavel|Texto') {
	//PEGA VARIACAO
	if ($specid>0 && $plantaid==0) {
		$sql2  = "SELECT mudaunidade(TraitVariation,'".$otipo."', TraitUnit,'".$newunit."') as vv FROM Traits_variation WHERE TraitID=".$row['TraitID']." AND EspecimenID=".$specid;
		//echo $sql2."<br >";
		$rr1 = mysql_query($sql2);
		$rw1 = mysql_fetch_assoc($rr1);
		$vv1 = $rw1['vv'];
	}
	if ($specid==0 && $plantaid>0) {
		$sql2  = "SELECT mudaunidade(TraitVariation,'".$otipo."', TraitUnit,'".$newunit."') as vv FROM Traits_variation WHERE TraitID=".$row['TraitID']." AND PlantaID=".$plantaid;
		$rr1 = mysql_query($sql2);
		$rw1 = mysql_fetch_assoc($rr1);
		$vv1 = $rw1['vv'];
		$sql2  = "SELECT mudaunidade(TraitVariation,'".$otipo."', TraitUnit,'".$newunit."') as vv FROM Monitoramento WHERE TraitID=".$row['TraitID']." AND PlantaID=".$plantaid;
		$rr2 = mysql_query($sql2);
		$rw2 = mysql_fetch_assoc($rr2);
		$vv2 = $rw2['vv'];
	}
	if ($specid>0 && $plantaid>0) {
		$sql2  = "SELECT mudaunidade(TraitVariation,'".$otipo."', TraitUnit,'".$newunit."') as vv FROM Traits_variation WHERE TraitID=".$row['TraitID']." AND (EspecimenID=".$specid." OR PlantaID=".$plantaid.")";
		$rr1 = mysql_query($sql2);
		$rw1 = mysql_fetch_assoc($rr1);
		$vv1 = $rw1['vv'];
		$sql2  = "SELECT mudaunidade(TraitVariation,'".$otipo."', TraitUnit,'".$newunit."') as vv FROM Monitoramento WHERE TraitID=".$row['TraitID']." AND PlantaID=".$plantaid;
	...
	if ($otipo=='Variavel|Quantitativo' || $otipo=='Variavel|Categoria') {
		$v1 = explode(";",$vv1);
		$v2 = explode(";",$vv2);
		$v1 = array_merge((array)$v1,(array)$v2);
	...
	$oformato = $row['formato'];
	//echopre($v1);

	if ($otipo=='Variavel|Quantitativo') {
		if (count($v1)>1) {
			//echopre($v1);
			$media = round(array_sum($v1)/count($v1),2);
			$somadesviso= 0;
			foreach ($v1 as $i) {
		        $somadesviso += pow($i - $media, 2);
		    }
	    	$odesvio = round(sqrt($somadesviso/(count($v1)-1)),2);
	    	...
	    	if ($oformato =='meansdrange') {
	    		$valor = $media."+/-".$odesvio." (".$minimo."-".$maximo.")";
	    	}
	    	if ($valor=="") {
	    		$valor = implode(";",$v1);
	    	}
		...
		if ($otipo=='Variavel|Categoria') {
			...
			foreach ($v1 as $i) {
				...
				$stn = mb_strtolower($rwq['TraitName']);
				$estados[] = $stn;
			}
			...
			if ($nst>2) { #se for maior que dois coloca categtxt entre os dois últimos
				...
				$pont1 = trim($row['categpontuacao']);
				$tt1 = implode($pont1." ", $arr1);
				$valor = $tt1." ".trim($row['categtxt'])."  ".trim($estados[($nst-1)]);
			} else {
				if ($nst==2) {
					$sep = "  ".trim($row['categtxt'])." ";
					$valor = implode($sep, $estados);
				} else {
					$valor = implode("",$estados);
				}
			}
			} else {
				$valor = $v1;
	...
	if (!empty($valor)) {
			//echo "o valor é".$valor."ate<br >";
			$prefixoprevio = $row['prefixoprevio'];
			$pega = "";
			if ($prefixoprevio=="on" && empty($valorprevio)) {
				$pega = trim($oprefixoprevio." ".$row['prefixo']);
				$oprefixoprevio = $pega;
			} else {
				$pega = trim($row['prefixo']);
				$oprefixoprevio = $row['prefixo'];
			}
			if (($otipo=='Variavel|Texto' || $otipo=='Variavel|Categoria') && empty($pega)) {
				$valor = trim($valor);
				if ($ucfirstvalue!="on") {
					$valor = ucfirst($valor);
				} else {
			...
			$linha = $pega." ".$valor." ".$row['sufixo'];
			$linha = trim($linha);
			if (empty($descricao)) { $linha = ucfirst($linha);}
			$descricao .= " ".$linha;
	} else {
		$prefixoprevio = $row['prefixoprevio'];
		if ($prefixoprevio=="on") {
...
function formulariomodelohabitat($habitatid,$formid,$specimenid) {
$sql = "SELECT tr.TraitID, tr.TraitTipo, prefixo , prefixoprevio,ucfirstvalue, sufixo , formato , unitformat , unitinclude ,namostral ,categpontuacao, categtxt  FROM FormulariosTraitsList  as ff JOIN Traits as tr USING(TraitID) WHERE FormID=".$formid." ORDER BY Ordem";
//echo $sql."<br />";
$res = mysql_query($sql);
$descricao = "";
...
while($row = mysql_fetch_assoc($res)) {
	...
	//PEGA VARIACAO
	if ($habitatid>0) {
		$sql2  = "SELECT mudaunidade(HabitatVariation,'".$otipo."', TraitUnit,'".$newunit."') as vv FROM Habitat_Variation WHERE TraitID=".$row['TraitID']." AND HabitatID=".$habitatid;
		//echo $sql2."<br >";
		$rr1 = mysql_query($sql2);
		$rw1 = mysql_fetch_assoc($rr1);
		$vv1 = $rw1['vv'];
	...
	if ($specimenid>0) {
		$sql3  = "SELECT mudaunidade(TraitVariation,'".$otipo."', TraitUnit,'".$newunit."') as vv FROM Traits_variation WHERE TraitID=".$row['TraitID']." AND EspecimenID=".$specimenid;
		$rr2 = mysql_query($sql3);
		$rw2 = mysql_fetch_assoc($rr2);
		$vv2 = $rw2['vv'];
	}	
	if ($otipo=='Variavel|Quantitativo' || $otipo=='Variavel|Categoria') {
...
if ($habitatid>0) {
	$sql = "SELECT hab.PathName as classe,taxalist(hab.EspeciesIds) as txlist FROM Habitat as hab WHERE HabitatID=".$habitatid;
	$rr1 = mysql_query($sql);
	$rw1 = mysql_fetch_assoc($rr1);
	//$classe = str_replace(".","",$rw1['classe']);
	$classe = trim($rw1['classe']);
	$spplist = trim($rw1['txlist']);
	if (!empty($classe)) {
		$descricao = $classe.".  ".$descricao;
	}
	if (!empty($spplist)) {
		$descricao .=  "Descatam-se: ".$spplist;
	}
}
$descricao = trim($descricao);
$descricao = str_replace(".,",".",$descricao);
...
function checklistplantasqq($daptraitid,$alturatraitid,$habitotraitid,$statustraitid,$duplicatesTraitID,$quickview,$quicktbname,$checkoleo=0,$traitsilica){
$qq = "SELECT 
pltb.PlantaID, 
pltb.DetID,
...
getlatlong(pltb.Latitude+0, pltb.Longitude+0, pltb.GPSPointID, pltb.GazetteerID, muni.MunicipioID, 0, 0, 0) as LONGITUDE,
getlatlong(pltb.Latitude+0, pltb.Longitude+0, pltb.GPSPointID, pltb.GazetteerID, muni.MunicipioID, 0, 0, 1) as LATITUDE,
IF(ABS(pltb.Longitude+0)>0,pltb.Altitude+0,IF(pltb.GPSPointID>0,gpspt.Altitude+0,IF(ABS(gaz.Longitude+0)>0,gaz.Altitude+0,NULL))) as ALTITUDE,
";
if ($daptraitid>0) {
	$qq .= " (traitvalueplantas(".$daptraitid.", pltb.PlantaID, 'mm', 0, 1)+0) AS DAPmm,";
}
if ($alturatraitid>0) {
	$qq .= " (traitvalueplantas(".$alturatraitid.", pltb.PlantaID, 'm', 0, 1))+0 AS ALTURA,";

}
if ($habitotraitid>0) {
	$qq .= " (traitvalueplantas(".$habitotraitid.", pltb.PlantaID, '', 0, 0)) AS HABITO,";
}
if ($statustraitid>0) {
	$qq .= " traitvalueplantas(".$statustraitid.",pltb.PlantaID, '', 0,0 ) AS STATUS,";
}
if ($checkoleo==1 && $traitsilica>0) {
	$qq .= "
checkoleo(0,pltb.PlantaID,".$traitsilica.",'oleo') AS OLEO, ";
}
$qq .= "
'mapping.png' AS MAP,
...
projetostringbrahmsnovo(0,pltb.PlantaID) as PROJETO,
if (checkimgs(0, pltb.PlantaID)>0,'camera.png','') as IMG,";
if ($duplicatesTraitID>0) {
	$qq .= " traitvalueplantas(".$duplicatesTraitID.", pltb.PlantaID, '', 0, 0) as DUPS,";
}
$qq .= "
...
function checklistspecsqq($herbariumsigla,$duplicatesTraitID,$daptraitid,$alturatraitid,$habitotraitid,$traitfertid,$quickview,$quicktbname, $checkoleo=0,$traitsilica) {
$qq = "SELECT 
pltb.GazetteerID,
pltb.GPSPointID,
pltb.EspecimenID, 
...
$qq .= "(IF(projetostringbrahmsnovo(pltb.EspecimenID,0)<>'',projetostringbrahmsnovo(pltb.EspecimenID,0),'NÃO FOI DEFINIDO')) as PROJETO";
if ($quickview>0 && !empty($quicktbname)) {
		...
		if ($traittipo=='Variavel|Quantitativo') {
			...
			if ($nv>1) {
				...
				if ($varunit!=GetLangVar('namenumero')) {
					$varname = trim($varname).$mean."+/-".$stdev." [".$minvar."-".$maxvar."] ".mb_strtolower($varunit);
				} else {
					$varname = trim($varname).$minvar."-".$mean."-".$maxvar." ".mb_strtolower($varunit);
				}
			} elseif ($nv==1) {
...
function updatetaxatraits($arraryofvalue,$bibkeyid,$typeid, $idd,$conn) {
$err =0;
foreach ($arraryofvalue as $key => $value) {
		$arraykey = explode("_",$key);
		$charid = $arraykey[1];
		...
		if (!empty($value) && $varorunit!='traitmulti' && $varorunit!='traitunit') {
			...
			$fieldname = "TraitVariation";
			$tbidfie = "TraitVariationID";
			$qq = "SELECT * FROM `".$tablename."` WHERE `TraitID`='".$charid."' AND BibkeyID='".$bibkeyid."' AND `".$typeid."`='".$idd."'";
			$teste = mysql_query($qq,$conn);
			$update = @mysql_numrows($teste);
		}
		$fieldsaskeyofvaluearray = array($typeid => $idd, 'BibkeyID' => $bibkeyid);
		$zz = array('TraitID' => $charid, 'TraitVariation' => $value, 'TraitUnit' => $ttunidade);
		$fieldsaskeyofvaluearray = array_merge((array)$fieldsaskeyofvaluearray,(array)$zz);
		if (!empty($value) && $value!=' ' && $update==0 && $varorunit!='traitmulti' && $varorunit!='traitimg' && $varorunit!='traitunit' && $charid>0) {
			$newtrait = InsertIntoTable($fieldsaskeyofvaluearray,
...
//Accordingly, a new image object is created called src_image.
	...
	$system=explode('.',$name);
	//print_r($system);
	$system[1] = mb_strtolower($system[1]);
	...
	if (empty($detaction) || empty($cooraction) || empty($localaction) || empty($habaction)) {
		...
		if ($taxon>0 || $local>0 || $hab>0 || !empty($coor)) {
			...
			if (!empty($coor) && empty($cooraction)) {
				...
				if ($coor=='planta') {
						echo "<tr class='tdsmallbold'><td colspan=100%> ($ii) ".GetLangVar('erro22')." (".GetLangVar('nametaggedplant')." + ".mb_strtolower(GetLangVar('namecompleta')).") ".GetLangVar('nameselect').":</td></tr>";
				} else {
						echo "<tr class='tdsmallbold'><td colspan=100%> ($ii) ".GetLangVar('erro22')." (".mb_strtolower(GetLangVar('namecoleta'))." + ".GetLangVar('namecompleta').") ".GetLangVar('nameselect').":</td></tr>";
				}
...
function CheckSamplesConflictforPlants($gazetteerid,$habitatid,$latitude,$longitude,$altitude,$altmin,$altmax,$especimensids,$conn) {
	...
	JOIN Country USING(CountryID) WHERE EspecimenID='".$id."'";
		...
		if ($nnomes>1 || $nlocais>1 || $ncoord>1 || $nhabitats>1) {
			...
			if ($nnomes>1) {
				echo " <b>".mb_strtolower(GetLangVar('nametaxonomy'))."</b>";
			} 
			if ($nlocais>1) {
				echo " <b>".mb_strtolower(GetLangVar('namelocalidade'))."</b>";
			}
			if ($ncoord>1) {
				echo " <b>".mb_strtolower(GetLangVar('namecoordenadas'))."</b>";
			}
			if ($nhabitats>1) {
				echo " <b>".mb_strtolower(GetLangVar('namehabitat'))."</b>";
			}
			echo "</td></tr>
			<tr><td colspan=2>
...
function TaxonomySimple($all=true,$conn) {
	...
	foreach ($qqarr as $kk => $sql) {
		if ($ii==0) {
			$qbse = "CREATE TABLE ".$tempname." (Familia CHAR(255) NULL, nome CHAR(255) NULL, nomeid CHAR(255) NULL, id INT(10) NOT NULL, FamiliaID INT(10) NULL, GeneroID INT(10) NULL, EspecieID INT(10) NULL, InfraEspecieID INT(10) NULL) CHARACTER SET utf8 ENGINE InnoDB ".$sql;
		} else {
...
getlocallatlong(0, gaz3.GazetteerID, gaz3.MunicipioID, 0)  AS logitude,NULL as alt, CONCAT('gazetteerid_',gaz3.GazetteerID) as nomeid, CURDATE()  as dataupdated  FROM Plantas JOIN GPS_DATA as gps ON GPSPointID=gps.PointID JOIN Gazetteer as gaz  ON gps.GazetteerID=gaz.GazetteerID  JOIN Gazetteer as gaz2 ON gaz2.GazetteerID=gaz.ParentID JOIN Gazetteer as gaz3 ON gaz3.GazetteerID=gaz2.ParentID JOIN Municipio as muni ON gaz.MunicipioID=muni.MunicipioID JOIN Province as prov ON prov.ProvinceID=muni.ProvinceID JOIN Country as crt ON crt.CountryID=prov.CountryID";
	...
	$qq = "CREATE TABLE IF NOT EXISTS  ".$tempname." ( nome VARCHAR(400), searchname VARCHAR(400) NULL, lat DOUBLE NULL, logitude DOUBLE NULL, alt DOUBLE NULL, nomeid CHAR(200) NULL, dataupdated DATE) CHARACTER SET utf8  ENGINE InnoDB"; 
mysql_query($qq,$conn);
//echo $qq."<br >";
...
function summarizeplantas($famid,$genusid,$speciesid,$infraspid,$conn) {
	...
	echo "<table class='tablethinborder' align='right' cellpadding=5>
		<tr class='clicked'>
				<td ><b>".$ngen."</b> &nbsp; ".mb_strtolower(GetLangVar('namegenus'))."s</td>
				<td>&nbsp;</td>
				<td ><b>".$nsps."</b>&nbsp;".mb_strtolower(GetLangVar('namespecies'))."</td>
				<td>&nbsp;</td>
				<td ><b>".$ninfsps."</b> &nbsp; ".mb_strtolower(GetLangVar('nameinfraspecies'))."</td>
				<td>&nbsp;</td>
				<td ><b>".$ngazs."</b>&nbsp; ".mb_strtolower(GetLangVar('namelocalidade'))."s</td>
		</tr>
		</table>";
											...
											if (count($amulti)>0) {
												...
												foreach ($amulti as $kk => $vvv) {
													...
													if (!empty($vvv)) {
														...
														$statename = mb_strtolower($catrow['TraitName']);
...
$tbname = "Temp_TaxaSummary_".$filtro;
$qq = "DROP TABLE ".$tbname;
@mysql_query($qq,$conn);

$qq = "CREATE TABLE ".$tbname." LIKE Especimenes (SELECT sp.* FROM Especimenes as sp JOIN FiltrosSpecs as fl ON sp.EspecimenID=fl.EspecimenID WHERE fl.FiltroID=".$filtro.")";
mysql_query($qq,$conn);
//$nsamp=0;

$qq = "SELECT * FROM Especimenes as sp JOIN FiltrosSpecs as fl ON sp.EspecimenID=fl.EspecimenID WHERE fl.FiltroID=".$filtro;
$rs = @mysql_query($qq,$conn);
$nsamp = mysql_num_rows($res);

$qq = "SELECT COUNT(DISTINCT Familia) as cfam FROM ".$tbname." as Ttax JOIN Identidade USING(DetID) JOIN Tax_Familias USING(FamiliaID) WHERE Familia<>'Indet'";
$res = @mysql_query($qq,$conn);
$rw = mysql_fetch_assoc($res);
$nfam = $rw['cfam'];

$qq = "SELECT COUNT(*) as nfamindet FROM ".$tbname." as Ttax JOIN Identidade USING(DetID) JOIN Tax_Familias USING(FamiliaID) WHERE Familia='Indet'";
$res = @mysql_query($qq,$conn);
$rw = mysql_fetch_assoc($res);
$nfamindet = $rw['nfamindet'];

$qq = "SELECT COUNT(DISTINCT Genero) as cgen FROM ".$tbname." as Ttax JOIN Identidade USING(DetID) JOIN Tax_Generos USING(GeneroID) WHERE Genero<>'Indet'";
$res = @mysql_query($qq,$conn);
$rw = mysql_fetch_assoc($res);
$ngen = $rw['cgen'];

$qq = "SELECT COUNT(*) as ngenindet FROM ".$tbname." as Ttax JOIN Identidade USING(DetID) JOIN Tax_Generos USING(GeneroID) WHERE Genero='Indet'";
$res = @mysql_query($qq,$conn);
$rw = mysql_fetch_assoc($res);
$ngenindet = $rw['ngenindet'];

$qq = "SELECT COUNT(DISTINCT Genero,Especie) as csp FROM ".$tbname." as Ttax JOIN Identidade USING(DetID) JOIN Tax_Generos ON Identidade.GeneroID=Tax_Generos.GeneroID JOIN Tax_Especies USING(EspecieID) WHERE Especie<>'Indet' AND Especie NOT LIKE 'sp.%'";
$res = @mysql_query($qq,$conn);
$rw = mysql_fetch_assoc($res);
$nsp = $rw['csp'];

$qq = "SELECT COUNT(*) as nspindet FROM ".$tbname." as Ttax JOIN Identidade USING(DetID) JOIN Tax_Generos ON Identidade.GeneroID=Tax_Generos.GeneroID JOIN Tax_Especies USING(EspecieID) WHERE Especie='Indet' OR Especie LIKE 'sp.%'";
$res = @mysql_query($qq,$conn);
$rw = mysql_fetch_assoc($res);
$nspindet = $rw['nspindet'];

$qq = "SELECT COUNT(DISTINCT Genero,Especie,InfraEspecie) as cspinf FROM ".$tbname." as tax JOIN Identidade USING(DetID) JOIN Tax_Generos ON Identidade.GeneroID=Tax_Generos.GeneroID JOIN Tax_Especies ON Identidade.EspecieID=Tax_Especies.EspecieID JOIN Tax_InfraEspecies USING(InfraEspecieID) WHERE InfraEspecie<>'Indet' AND InfraEspecie NOT LIKE 'sp.%' AND Identidade.InfraEspecieID>0";
$res = @mysql_query($qq,$conn);
$rw = mysql_fetch_assoc($res);
$cspinf = $rw['cspinf'];


$qq = "SELECT COUNT(*) as ninfindet FROM ".$tbname." as tax JOIN Identidade USING(DetID) JOIN Tax_Generos ON Identidade.GeneroID=Tax_Generos.GeneroID JOIN Tax_Especies ON Identidade.EspecieID=Tax_Especies.EspecieID JOIN Tax_InfraEspecies USING(InfraEspecieID) WHERE (InfraEspecie='Indet' OR InfraEspecie LIKE 'sp.%') AND Identidade.InfraEspecieID>0";
$res = @mysql_query($qq,$conn);
$rw = mysql_fetch_assoc($res);
...
function getfertility($specid,$traitfertid,$conn) {
	...
	if (!empty($var)) {
		...
		foreach ($arvar as $vv) {
			...
			$var = mb_strtolower($rrs['TraitName']);
					...
					if (!empty($vvar)) {
						if ($traittipo=='Variavel|Categoria') {
							...
							foreach ($aarvar as $kk => $val) {
								...
								$varsing = mb_strtolower($rwo['TraitName']);
					...
					if ($traittipo=='Variavel|Quantitativo') {
							...
							if ($nv>1) {
								...
								if (substr($varunit,0,2)!='nu') {
									$varname = $mean."+/-".$stdev." [".$minvar."-".$maxvar."] ".mb_strtolower($varunit);
								} else {
									$varname = $minvar."-".$mean."-".$maxvar." ".mb_strtolower($varunit);
								}
...
function GetTaxaData($typeid,$idd,$bibkey,$formid,$conn) {
	if ($formid>0) {
		...
		$res = mysql_query($qq,$conn);
		$rw = mysql_fetch_assoc($res);
		$traitids = explode(";",$rw['traitids']);
		$nn=1;
	} else {
		$qq = "SELECT TraitID FROM Traits_variation WHERE ".$typeid."='".$idd."' AND BibkeyID='".$bibkey."'";
		$rr = mysql_query($qq,$conn);
		$nn = mysql_numrows($rr);
		$traitids = array();
		while ($rw = mysql_fetch_assoc($rr)) {
	...
	if ($nn>0) {
		foreach ($traitids as $ttid) {
			$qq = "SELECT * FROM Traits_variation WHERE ".$typeid."='".$idd."' AND TraitID='".$ttid."'  AND BibkeyID='".$bibkey."'";
			$resul = mysql_query($qq,$conn);
			$nnn = mysql_numrows($resul);
			$row = mysql_fetch_assoc($resul);
			$charid = $row['TraitID'];
		...
		foreach ($arrofdates as $data) {
			...
			if ($nrr==1) {
					...
					if ($traittipo=='Variavel|Categoria' && $multsel=='Sim') {
						...
						$res = mysql_query($qq,$conn);
						$resw = mysql_fetch_assoc($res);
						$varname = mb_strtolower($resw['TraitName']);
					}
					...
					if (!empty($vvar)) {
						if ($traittipo=='Variavel|Categoria') {
							...
							foreach ($aarvar as $kk => $val) {
								...
								$rwo = mysql_fetch_assoc($ror);
								if ($english) {
									$varsing = mb_strtolower($rwo['TraitName_English']);
									$vs = $varsing;
			...
			if (count($variation)>0) {
					...
					if ($traittipo=='Variavel|Quantitativo') {
							...
							if ($nv>1) {
								...
								if ($vvunit=='NUMERO') { $vu = '';} else { $vu = mb_strtolower($varunit);}
...
function printtraitname($currentpathname,$previouspathname,$currentclassadj,$jaindumento,$conn) {
		...
		$cct = mb_strtolower($currenttrait);
		$cct = strtloweracentos($cct);
		$resultado .= $cct;
							...
							if (count($variation)>0) {
									...
									if ($traittipo=='Variavel|Quantitativo') {
											...
											if ($nv>1) {
												...
												if ($stdev>0 && $nv>2) { $sdv = "+/-".$stdev; } else { $sdv='';}
												if ($nv==2 && $minvar==$maxvar) { $svv=''; } else { $svv=" [".$minvar."-".$maxvar."] ";}
												if (substr($varunit,-1,4)=='mero') { $vu = '';} else { $vu = mb_strtolower($varunit);}
												$varname = $mean.$sdv.$svv.$vu;
												if ($printN) {
													$varname .= " (N=".$nv.")";
												}
...
function listspecimens_rodriguesia($nome,$list_measured,$list_notmeasured,$typeid,$desctemptable,$conn) {
			...
			while ($rsw = mysql_fetch_assoc($rss)) {
				...
				if (!empty($longitude)) {
				    ...
				    $coords = $latt." ".mb_strtolower(GetLangVar('nameand'))." ".$longtt;
				    if (!empty($altitude)) {
		...
		foreach ($newtraitarray as $tids => $specids) {
			...
			if ($traittipo=='Variavel|Categoria') {
				...
				while ($ruqq = mysql_fetch_assoc($ruq)) {
					if ($english) {
						$rru = mb_strtolower($ruqq['TraitName_English']);
					} else {
						$rru = mb_strtolower($ruqq['TraitName']);
				...
				foreach ($traitstobreak as $trbr) {
					...
					if ($traittipo=='Variavel|Categoria') {
						...
						while ($ruqqw = mysql_fetch_assoc($ruqq)) {
							if ($english) {
								$rruw = mb_strtolower($ruqqw['TraitName_English']);
							} else {
								$rruw = mb_strtolower($ruqqw['TraitName']);
