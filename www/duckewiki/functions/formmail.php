...
define("DEF_ALERT","vicentini.beto@gmail.com");
            ...
            if (count($a_toks) > 0)
            {
                ...
                case "lcase":
                    ...
                    elseif ($a_toks[0] == "ucase")
                        ...
                        $s_value = mb_strtolower(GetFieldValue($a_toks[2],$a_form_data));
                    break;
                case "ltrim":
                case "rtrim":
                case "trim":
...
function CheckValidURL($s_url)
{
    ...
    foreach ($TARGET_URLS as $s_prefix)
        if (!empty($s_prefix) &&
                mb_strtolower(substr($s_url,0,strlen($s_prefix))) ==
                mb_strtolower($s_prefix))
            return (true);
    return (false);
}
...
function DetectJunkAttack($a_fields,&$s_attack,&$s_info,&$s_user_info)
{
    ...
    foreach ($a_fields as $s_fld=>$m_value)
    {
        ...
        if (isset($m_value) && !FieldManager::IsEmpty($m_value))
        {
            ...
            {
                $s_orig_data = $s_data = mb_strtolower($s_data);
                    //
