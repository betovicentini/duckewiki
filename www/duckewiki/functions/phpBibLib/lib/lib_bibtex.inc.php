...
class Bibtex {
	...
	function ParseEntry($rb) {
		...
		$entry['type'] = mb_strtolower(substr($rb, 1, $pa-1));
		...
		foreach($rawfields as $field) {
			$fieldname = mb_strtolower(trim(substr($field, 0, strpos($field, '='))));
	...
	function CmpString($a, $b, $d) {
		return strcmp(($d == 'a' ? mb_strtolower($a) : mb_strtolower($b)), ($d == 'a' ? mb_strtolower($b) : mb_strtolower($a)));
	}
