...
class Structures_BibTex
{
    ...
    function _parseEntry($entry)
    {
        ...
        $ret = array();
        if ('@string' ==  mb_strtolower(substr($entry, 0, 7))) {
            //String are not yet supported!
            if ($this->_options['validate']) {
                $this->_generateWarning('STRING_ENTRY_NOT_YET_SUPPORTED', '', $entry.'}');
            }
        } elseif ('@preamble' ==  mb_strtolower(substr($entry, 0, 9))) {
            ...
            while (strrpos($entry,'=') !== false) {
                ...
                $position    = strrpos($entry, ',');
                $field       = mb_strtolower(trim(substr($entry, $position+1)));
            ...
            $arr = explode('{', $entry);
            $ret['cite'] = trim($arr[1]);
            $ret['entryType'] = mb_strtolower(trim($arr[0]));
            if ('@' == $ret['entryType']{0}) {
    ...
    function bibTexEntry($entry)
    {
        $bibtex = '';
        //if ($entry = $this->data[$bibkey];
            //Intro
            $bibtex .= '@'.strtolower($entry['entryType']).' { '.$entry['cite'].",\n";
            //Other fields except author
            foreach ($entry as $key=>$val) {
