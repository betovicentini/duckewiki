...
if (!isset($usarmodelohabitat)) { $usarmodelohabitat=0;}
...
pltb.HabitatID AS WikiHabitatID, 
...
if ($duplicatesTraitID>0) {
	$qqbrahms .=", traitvaluespecs(".$duplicatesTraitID.",0,pltb.EspecimenID,'',0,0) as initial";
} else {
	$qqbrahms .=", '' as initial";
}
$qqbrahms .=", 
...
if ($localidadetraitid>0) {
	$qqbrahms .=", IF (localidadefields(pltb.GazetteerID, pltb.GPSPointID,pltb.MunicipioID, pltb.ProvinceID,pltb.CountryID, 'GAZminusParent')<>'',
CONCAT(localidadefields(pltb.GazetteerID, pltb.GPSPointID,pltb.MunicipioID, pltb.ProvinceID,pltb.CountryID, 'GAZminusParent'),'. ',traitvaluespecs(".$localidadetraitid.",0,pltb.EspecimenID,'',0,0)),
traitvaluespecs(".$localidadetraitid.",0,pltb.EspecimenID,'',0,0)) as locnotes";
} else {
	...
	if ($res) {
		if ($step==0) {
			...
			for ($i = 0; $i < $count; $i++){
				...
				$acabeca[] = $cl;
				if (($cl!='WikiPlantaID' && $cl!='WikiHabitatID') || $forbrahms<>1) {
					if ($i<($count-1)) {
		...
		while($rsw = mysql_fetch_row($res)){
			...
			$nff  = count($rsw);
			$nii = 1;
			foreach($rsw as $coluna => $value){
				if (($acabeca[$coluna]!='WikiPlantaID' && $acabeca[$coluna]!='WikiHabitatID') || $forbrahms<>1) { 
				if ($acabeca[$coluna]=='plantdesc' && $forbrahms==1 && $usarmodelo==1 && $formnotas>0) {
					$aspecid = $rsw[0];
					$aplantaid = $rsw[1];
					$value = formulariomodelo($aspecid,$aplantaid,$formnotas);
					//echo "aqui".$value."<br >";
				}
				if ($acabeca[$coluna]=='habitattxt' && $forbrahms==1 && $usarmodelohabitat==1 && $formidhabitat>0) {
					$ahabitatid = $rsw[2];
					$aspecid = $rsw[0];
					$value = formulariomodelohabitat($ahabitatid,$formhabitatdesc,$aspecid);
					//echo $value." aqui<br />";
				}
...
<form action='export-especimenes-exec.php' name='myform' method='post'>
  ...
  <input type='hidden' name='usarmodelo'  value='".$usarmodelo."' />
<table align='left' cellpadding='5' width='50%' class='erro'>
  <tr><td>Processando passo ".($step+1)." de ".($nsteps+1)."  AGUARDE!</td></tr>
  <tr><td>Falta aproximadamente ".$tfalta."  minutos para terminar!</td></tr>
</table>
