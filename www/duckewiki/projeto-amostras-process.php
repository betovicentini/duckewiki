...
$rr =  mysql_query("SELECT * FROM `".$tbname."` WHERE EspecimenID=".$spid."  AND ProjetoID=".$projetoid);
...
if ($nrr>0) {
  ...
  if ($status==1) {
        $qins = "INSERT INTO  `".$tbname."` (`EspecimenID`,`ProjetoID`,`AddedBy`,`AddedDate`) VALUES (".$spid.",'".$projetoid."', '".$uuid."', CURRENT_DATE())";
...
$rr =  mysql_query("SELECT * FROM `".$tbname."` WHERE EspecimenID>0 AND ProjetoID=".$projetoid);
