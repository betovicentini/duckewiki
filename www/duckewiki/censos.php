...
if (!isset($datainicio) && !isset($datafim) && ($filtro+0)>0) {
	...
	$qz = "SELECT COUNT(*) as npl FROM Plantas AS pl JOIN FiltrosSpecs as f ON pl.PlantaID=f.PlantaID WHERE f.FiltroID =".$filtro;
...
";	
	...
	$qz = "SELECT DISTINCT CensoID FROM Monitoramento as m JOIN FiltrosSpecs as f ON m.PlantaID=f.PlantaID WHERE f.FiltroID =".$filtro." AND CensoID>0";
	$rz = @mysql_query($qz,$conn);
        ...
        <td>
          <select name='censos' multiple size='5' readonly>";
			while ($rr = @mysql_fetch_assoc($rz)) {
				...
				$qz = "SELECT COUNT(DISTINCT TraitID) as trs FROM Monitoramento m JOIN FiltrosSpecs as f ON m.PlantaID=f.PlantaID WHERE f.FiltroID =".$filtro." AND CensoID='".$rr['CensoID']."'";
				$rzk = @mysql_query($qz,$conn);
				$rwzk = @mysql_fetch_assoc($rzk);
				$trs = $rwzk['trs'];
				$qz = "SELECT COUNT(DISTINCT PlantaID) as pls FROM Monitoramento m JOIN FiltrosSpecs as f ON m.PlantaID=f.PlantaID WHERE f.FiltroID =".$filtro." AND CensoID='".$rr['CensoID']."'";
				$rzk = @mysql_query($qz,$conn);
				$rwzk = @mysql_fetch_assoc($rzk);
				$pls = $rwzk['pls'];
      ...
      <tr>
        ...
        <td>
          ...
          <option selected value=''>Selecione das medições</option>";
			$qzz = "SELECT DISTINCT DataOBS FROM Monitoramento m JOIN FiltrosSpecs as f ON m.PlantaID=f.PlantaID WHERE f.FiltroID =".$filtro."  AND CensoID=0 ORDER BY DataOBS ASC";
      ...
      <tr>
        ...
        <td>
          <select name='datafim'>";
			echo "
          <option selected value=''>Selecione das medições</option>";
			$qzz = "SELECT DISTINCT DataOBS FROM Monitoramento m JOIN FiltrosSpecs as f ON m.PlantaID=f.PlantaID WHERE f.FiltroID =".$filtro." AND CensoID=0 ORDER BY DataOBS DESC";
			$rzz = mysql_query($qzz,$conn);
			while ($rrr = @mysql_fetch_assoc($rzz)) {
				echo "
          <option value='".$rrr['DataOBS']."'>".$rrr['DataOBS']."</option>";
...
if ((!empty($datainicio) || !empty($datainicio_nova)) && (!empty($datafim) || !empty($datafim_nova)) && ($filtro+0)>0 && ($formularioid+0)>0) {
	...
	if ($censoset>0) {
		...
		foreach ($traits as $vv)  {
			$qzz = "UPDATE Monitoramento as moni, FiltrosSpecs as pl SET moni.CensoID='".$censoset."' WHERE pl.PlantaID=moni.PlantaID AND pl.FiltroID=".$filtro." AND moni.DataOBS>='".$datainicio."' AND  moni.DataOBS<='".$datafim."' AND moni.TraitID='".$vv."'";
			$rzz = @mysql_query($qzz,$conn);
			if ($rzz) {
				$tr++;
			}
