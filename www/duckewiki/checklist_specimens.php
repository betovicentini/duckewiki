...
if (empty($tbname)) {
	if ($filtro>0 || $detid>0 || $quickview>0 || $plantaid>0) {
		$tbname = "temp_listspecs".substr(session_id(),0,10);
		$update=1;
	} else {
...
$qqz = "CREATE TABLE `temp_progspec".$tbname."`  (`percentage` INT(10) NOT NULL) ENGINE = InnoDB CHARACTER SET utf8 COLLATE utf8_general_ci";
mysql_query($qqz,$conn);
$qqz = "INSERT INTO `temp_progspec".$tbname."` (`percentage`) VALUES ('0');";
mysql_query($qqz,$conn);
...
"<script>
    function CheckProgress() {
      ...
      $.get('checklist_specimens_progress.php', { t: time, tbname: '".$tbname."'}, function (data) {
        ...
        if (progress < 100) {
          ...
          window.location = 'checklist_specimens_save.php?tbname=".$tbname."&ispopup=1';
    	}
      });
	}
    ...
    $.ajax(
            {
                type: 'GET',
                url: 'checklist_specimens_script.php',
                data: '".$varstxt."',
                async: true,
                success:
                    function (data) {
...
$body='';
$title = 'Preparando o Checklist de specimenes!';
//FAZ O CABECALHO
