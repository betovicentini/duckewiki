...
if ($final==1) {
	...
	if ($filtro>0 && $groupid>0) {
		...
		if (count($members)>0) {
			...
			$sql = "UPDATE Plantas INNER JOIN Identidade USING (DetID)  JOIN FiltrosSpecs as fl ON Plantas.PlantaID=fl.PlantaID  SET Plantas.GruposSppIDs=IF(Plantas.GruposSppIDs<>'',CONCAT(Plantas.GruposSppIDs,';','".$grupotag."'),'".$grupotag."') WHERE (".$qq.") AND fl.FiltroID=".$filtro." AND Plantas.GruposSppIDs NOT LIKE '%".$grupotag."%'";
			...
			$sql = "UPDATE Especimenes INNER JOIN Identidade USING (DetID)  JOIN FiltrosSpecs as fl ON Especimenes.EspecimenID=fl.EspecimenID SET Especimenes.GruposSppIDs=IF(Especimenes.GruposSppIDs<>'',CONCAT(Especimenes.GruposSppIDs,';','".$grupotag."'),'".$grupotag."') WHERE (".$qq.") AND fl.FiltroID=".$filtro." AND Especimenes.GruposSppIDs NOT LIKE '%".$grupotag."%'";
			$updatedpls = mysql_query($sql,$conn);
			if ($updatedpls) {
				$salvo++;
			} else {
...
if ($groupid>0) {
	...
	echo mb_strtolower(GetLangVar('namespeciesgroups'))." 
  </td>
</tr>
</thead>
<tbody>";
