...
localidadestring(pltb.GazetteerID,pltb.GPSPointID,pltb.MunicipioID,pltb.ProvinceID,pltb.CountryID,pltb.Latitude+0,pltb.Longitude+0,pltb.Altitude+0) as LOCALIDADE,
...
getlatlongdms(pltb.Latitude+0, pltb.Longitude+0, pltb.GPSPointID+0, pltb.GazetteerID+0, pltb.MunicipioID+0, pltb.ProvinceID+0, pltb.CountryID+0, 1,5)  as LATITUDE,
...
IF(ABS(pltb.Longitude+0)>0,pltb.Altitude+0,IF(pltb.GPSPointID>0,gpspt.Altitude+0,IF(ABS(gaz.Longitude)>0,gaz.Altitude,''))) as ALTITUDE,
...
IF(pltb.HabitatID>0,pltb.HabitatID,IF((plspectb.HabitatID+0)>0,plspectb.HabitatID+0,0)) as HabitatID,
...
labeldescricao(pltb.EspecimenID+0,pltb.PlantaID+0,(".$formnotes."+0),FALSE,FALSE) as NOTAS";
...
//IF(ABS(pltb.Longitude)>0,pltb.Latitude,IF(pltb.GPSPointID>0,gpspt.Latitude,IF(ABS(gaz.Longitude)>0,gaz.Latitude,muni.Latitude))) as LATITUDE,	
	if ($filtro>0) {
	   $qq .=   " JOIN FiltrosSpecs as fl ON pltb.EspecimenID=fl.EspecimenID WHERE fl.FiltroID=".$filtro;
		...
		$qz = "SELECT * FROM Especimenes JOIN FiltrosSpecs as fl ON Especimenes.EspecimenID=fl.EspecimenID WHERE fl.FiltroID=".$filtro;
...
while ($ruqw = mysql_fetch_assoc($ruq)) {
	...
	foreach ($imgs as $vimg) {
		...
		if (!file_exists($pathcpbres)) {
		    $opath = $urlbig.$imgn;
		    $ourllow = $urllowres.$imgn;
		} else {
		    $opath = $urlbig.$rusqw['FileName'];
		    $ourllow = $urllowres.$rusqw['FileName'];
		}
		//////
		$tutx = "
<tr><td><a href=\"".$opath."\"><img src=\"".$ourllow."\" width='200'></a><br></td></tr>";
		$txt .= $tutx;
	}
}
$txt .= "
