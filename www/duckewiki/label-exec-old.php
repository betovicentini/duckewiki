...
if (!empty($filtro)) { 
	...
	if ($etitype =='EspecimenesIDS') { 
		$tbname = 'Especimenes';
		$tbname2 = 'Plantas';
		$qwheretb =   " JOIN FiltrosSpecs as fl ON maintb.EspecimenID=fl.EspecimenID WHERE fl.FiltroID=".$filtro;
	} else {
		$tbname = 'Plantas';
		$tbname2 = 'Especimenes';
		$qwheretb =   " JOIN FiltrosSpecs as fl ON maintb.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtro;
...
if (isset($tbname) && !isset($prepared)) {
	...
	if ($etitype =='EspecimenesIDS') { 
		$qu = "SELECT COUNT(*) nrecs FROM Especimenes JOIN FiltrosSpecs AS fl ON Especimenes.EspecimenID=fl.EspecimenID WHERE fl.FiltroID=".$filtro;
	
		$qq .= " LEFT JOIN Pessoas as colpessoa ON maintb.ColetorID=colpessoa.PessoaID";
	} 
	else {
		$qu = "SELECT COUNT(*) nrecs FROM Plantas  JOIN FiltrosSpecs AS fl ON Plantas.PlantaID=fl.PlantaID WHERE fl.FiltroID=".$filtro;
	}
