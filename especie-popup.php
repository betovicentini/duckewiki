...
if ($speciesid>0 && $final!=1) {
	...
	$ttid = GetLangVar('nameeditar')."&nbsp;".strtolower(GetLangVar('namespecies'))." &nbsp;<i>$spnome</i>";
} //if editing
else {
	$ttid = GetLangVar('namenova')."&nbsp;".strtolower(GetLangVar('namespecies'));
}
...
<br />";
	...
	if ($erro==0) {
		...
		if ($speciesid>0) {
			$qq = "SELECT GeneroID,Especie,EspecieAutor,BibID,Basionym,BasionymAutor,PubRevista,PubVolume,PubAno,Sinonimos,GeoDistribution,Notas FROM Tax_Especies WHERE EspecieID=".$speciesid;
...
<br />";
		...
		//CASO NAO ESTEJA EDITANDO, ETNAO É NOVO CADASTRO
			...
			$qq = "SELECT * FROM Tax_Especies WHERE LOWER(Especie) LIKE '".strtolower($spnome)."' AND GeneroID='".$genusid."'";
			$qu = mysql_query($qq,$conn);
...
<thead>
  <tr><td >".$ttid."</td></tr>
  <tr class='subhead'><td>Checar dados da espécie $genero $spnome em www.ipni.org?</td></tr>
</thead>
<tbody>";
if ($bgi % 2 == 0){$bgcolor = $linecolor2 ;} else {$bgcolor = $linecolor1 ;} $bgi++;
echo "
...
// <a href='".$redirect."' target='_new' >Tropicos.org</a>
		echo "
