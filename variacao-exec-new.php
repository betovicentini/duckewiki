...
if ($option1=='2') {
	...
	$arval['final'], $arval['actionfilereset']);
	$result = array();
...
foreach ($arraryofvalue as $key => $value) {
	...
	//se vazio coloca um valor em branco para apagar registros
		if (empty($vv)) {$value=' ';}
		//pega os valores quando nao for vazio
		if ($varorunit=='traitvar' && !empty($value) && $value!=' ') {
			if (is_array($value)) {
				$value = implode(";",$value);
			} 
		}
		//processa imagens se for o caso
...
if (count($newimagefile)>0) {
		...
		'linkto' => $_POST['linkto'],
		'formid' => $_POST['formid'],
		'actionfilereset' => $_POST['actionfilereset']
		);

		$zz = explode("/",$_SERVER['SCRIPT_NAME']);
		$serv = $_SERVER['SERVER_NAME'];
		$returnto = $serv."/".$zz[1]."/variacao-exec-new.php";
...
<tr><td>Houve um erro no cadastro</td></tr>
</table>";
	} else {
		if ($imgdone>1) {
...
<br>
<table cellpadding='7' align='center' class='erro'>
<tr><td>Não foi possível gerar thumbnails para as imagens</td></tr>
</table>";
		}
		echo "
<br>
<table cellpadding='7' align='center' class='sucessosmall'>
