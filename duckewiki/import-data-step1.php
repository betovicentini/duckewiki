    ...
    <select name='tagnumfield'>
      <option value=''>".GetLangVar('nameselect')."</option>";
			...
			while ($rw = mysql_fetch_assoc($rq)) {
				...
				$fieldname = $zz[$xt];
			if ($fieldname!="ImportID") {
				$qq = "SELECT * FROM Import_Fields WHERE LOWER(NamesToMatch) LIKE '%".strtolower($fieldname)."%' AND FieldsToPut='PlantaTag'";
        ...
        <td>
          <select name='coletorfield'>
			...
			while ($rw = mysql_fetch_assoc($rq)) {
				...
				$fieldname = $zz[$xt];
			if ($fieldname!="ImportID") {
				$qq = "SELECT * FROM Import_Fields WHERE LOWER(NamesToMatch) LIKE '%".strtolower($fieldname)."%' AND FieldsToPut='ColetorID'";
				$rqq = mysql_query($qq,$conn);
				$nrqq = mysql_numrows($qq,$conn);
				if ($nrqq>0) {
					$ch = 'selected'; 
        ...
        <td>
          <select name='numcolfield'>
			...
			while ($rw = mysql_fetch_assoc($rq)) {
				...
				$fieldname = $zz[$xt];
			if ($fieldname!="ImportID") {
				$qq = "SELECT * FROM Import_Fields WHERE LOWER(NamesToMatch) LIKE '%".strtolower($fieldname)."%' AND FieldsToPut='Number'";
				$rqq = mysql_query($qq,$conn);
				$nrqq = mysql_numrows($qq,$conn);
				if ($nrqq>0) {
					$ch = 'selected'; 
