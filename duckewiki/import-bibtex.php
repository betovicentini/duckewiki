...
echo "
<br />
<form enctype='multipart/form-data' action='import-bibtex.php' method='post'>
...
mysql_query($qc, $conn);
		...
		foreach ($bibtex->data  as $vv) {
			...
			if ($erro==0) {
				...
				$qch = "SELECT * FROM BiblioRefs WHERE LOWER(Type)='".strtolower($vv['entryType'])."' AND year=".$yy." AND 
				FirstAuthor='".$lastn."'  ";
				if (!empty($pg)) {
				 	$qch .= " AND LOWER(Pages)='".strtolower($pg)."'";
				 }
				if (!empty($vl)) {
				 	$qch .= " AND (Volume)='".$vl."'";
				 } else {
				 	if (!empty($bk)) {
					 	$qch .= " AND LOWER(BookTitle)='".strtolower($bk)."'";
				 	} elseif (!empty($titu)) {
				 		$qch .= " AND LOWER(title)='".strtolower($titu)."' ";
				 	}
				 }
				...
				$qchh = "SELECT * FROM BiblioRefs WHERE LOWER(BibKey)='".strtolower($bibkey)."'";
			...
			if ($erro==0) {
				$inserted++;
				//echo "////////////////////////////////////// REGISTRO".$vv['cite']." IMPORTADO!<br />";
				//echo $qch."<br />";
				//echo "Bibkey inserida: ".$bibkey."  <br /><textarea>".$txtbib."</textarea><br />";
			} else {
				$erros++;
				echo "////////////////////////////////////// REGISTRO".$vv['cite']."<br />";
				echo "<font style='color: red'>ERRO: ".$errotxt."</font><br />BibTex não importado:<br /><textarea>".$txtbiborg."</textarea><br />";
