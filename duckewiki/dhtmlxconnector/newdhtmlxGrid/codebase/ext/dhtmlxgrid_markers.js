...
dhtmlXGridObject.prototype.doMark=function(a,b){var c=a.parentNode.idd,d=a._cellIndex;b||(b=0);if(b==0)this.unmarkAll();else if(b==1){if(this.lastMarkedRow)for(var h=Math.min(this.getRowIndex(c),this.getRowIndex(this.lastMarkedRow)),i=Math.max(this.getRowIndex(c),this.getRowIndex(this.lastMarkedRow)),j=Math.min(d,this.lastMarkedColumn),k=Math.max(d,this.lastMarkedColumn),e=h;e<i+1;e++)for(var f=j;f<k+1;f++)this.mark(this.getRowId(e),f,!0)}else if(b==2&&this.markedRowsArr._dhx_find(c)!=-1)for(var g=
0;g<this.markedCellsArr[c].length;g++)if(this.markedCellsArr[c][g]==d)return this.mark(c,d,!1),!0;this.markedCellsArr[c]||(this.markedCellsArr[c]=new dhtmlxArray(0));b!=1&&this.mark(c,d);this.moveToVisible(this.cells(c,d).cell);this.lastMarkedRow=c;this.lastMarkedColumn=d;this.lastMarkMethod=b};
...
dhtmlXGridObject.prototype.getMarked=function(){for(var a=[],b=0;b<this.markedRowsArr.length;b++)for(var c=this.markedRowsArr[b],d=0;d<this.markedCellsArr[c].length;d++)a[a.length]=[c,this.markedCellsArr[c][d]];return a};
