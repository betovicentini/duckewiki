<?php
//este script checa variaveis de monitoramento checando por compatibilidade
session_start();
...
if (in_array('MONI_VAR',$fields) && !isset($var_moni_ok)) {
	...
	if (!isset($varmonitors)) {
		...
		foreach ($monitorvars as $monikk => $vv) {
			...
			if ($traitid>0 && (!empty($date1) || !empty($date2))) {
				$colname = $tbprefix."MONITORAMENTO_".$vv."_traitvar_".$traitid;
				$colname2 = $tbprefix."MONITORAMENTO_".$vv."_dataobs_".$traitid;
				$qq = "ALTER TABLE ".$tbname." ADD COLUMN ".$colname." VARCHAR(1000) DEFAULT '', ADD COLUMN ".$colname2." DATE DEFAULT NULL".$qtt;
				@mysql_query($qq,$conn);
				if ($ttipo=='Categoria') {
...
<table cellpadding=\"1\" width='50%' align='center' class='erro'>
  <tr class='tdsmallbold' ><td align='center'>Não foi informada a data para a coluna $vv</td></tr>
    ...
    echo "<form action='import-habitat-step4.php' method='post'>";
...
<input name='tbname' value='".$tbname."' type='hidden' />
  <tr bgcolor = '".$bgcolor."'><td align='center' colspan='100%'><input type='submit' value='".GetLangVar('namecontinuar')."' class='bsubmit' /></td></tr>
				...
				echo "
<form name='lastform' action='import-habitat-step4.php' method='post'>";
    unset($_POST['varmonitors']);
foreach ($_POST as $kfs => $vfs) {
		if (!empty($vfs)) {
		echo "
...
<tbody>
<form name='monitorform' action='import-habitat-step4.php' method='post'>";
...
echo "
  <td align='center'>
    <table>
      <tr>
        <td><input type='hidden' name='monivarnovo_".$moni."' id='monivarnovo_".$moni."' value='' /><input type='text' id='monitornome_".$moni."' value='' size='10' readonly /></td>
        <td><img src='icons/list-add.png' height=15 ";
$myurl ="traitsnew-popup.php?traitname_val=monitornome_".$moni."&traitid_val=monivarnovo_".$moni."&fname=".$fieldname;
