...
'imagens' => "<img src=\"icons/camera.png\" ".$hgt." style=\"".$stilo."\" onmouseover=\"Tip('Importa e/ou relaciona imagens');\" onclick = \"javascript:small_window('imagens-form.php?ispopup=0',500,300,'Importa e/ou relaciona imagens');\" />"

);
if (empty($quais) || count($quais)==0) {
...
function TaxonomySimple($all=true,$conn) {
	...
	if ($all) {
		...
		$qqarr[] =  "(SELECT Familia, Familia as nome, CONCAT('famid_',FamiliaID) as nomeid, FamiliaID as id, FamiliaID, NULL as GeneroID, NULL as EspecieID, NULL as InfraEspecieID FROM Tax_Familias WHERE Familia<>'Indet')";
		$qqarr[] =  "(SELECT  Familia, Genero as nome, CONCAT('genusid_',GeneroID) as nomeid, GeneroID as id, Tax_Familias.FamiliaID, GeneroID, NULL as EspecieID, NULL as InfraEspecieID  FROM Tax_Generos JOIN Tax_Familias USING(FamiliaID) WHERE Genero<>'Indet')";
	...
	$qq = "SELECT DISTINCT Familia,nome,nomeid, id, FamiliaID, GeneroID, EspecieID, InfraEspecieID FROM ".$tempname." WHERE nome<>'Indet' ORDER BY Familia,nome";
