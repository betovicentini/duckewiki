...
if (count($basicvariables)>0) {
	...
	if ($formvariables>0) {
		$qu = "SELECT * FROM Traits WHERE FormulariosIDS LIKE '%formid_".$formvariables."%'";
	...
	if ($formhabitat>0) {
			$qu = "SELECT * FROM Traits WHERE FormulariosIDS LIKE '%formid_".$formhabitat."%'";
	...
	$qq .= " WHERE FiltrosIDS LIKE '%filtroid_".$filtro."%'";
