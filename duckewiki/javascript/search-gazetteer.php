...
<?php
	...
	$getRecord_sql	=	"
	SELECT DISTINCT CONCAT(gaz.PathName,' [',Municipio,'- ',Province,' - ',Country,']') as nome, gaz.GazetteerID as nomeid FROM Gazetteer as gaz JOIN Municipio  USING(MunicipioID) JOIN Province USING(ProvinceID) JOIN Country USING(CountryID) WHERE LOWER(CONCAT(gaz.PathName,' ',Municipio,' ',Province,' ',Country)) LIKE '%".$searchq."%' ORDER BY gaz.PathName";
