...
if ($final==2) {
	$qq = "SELECT * FROM Filtros WHERE  FiltroID=".$_POST['filtro'];
	$res = mysql_query($qq,$conn);
	$row = mysql_fetch_assoc($res);
	$specs = explode(";",$row['EspecimenesIDS']);
	$plids = explode(";",$row['PlantasIDS']);
	$fid = $row['FiltroID'];
	$filtronome = 'filtroid_'.$fid;
	if (count($plids)>0) {
		$sql = "UPDATE Plantas SET Plantas.FiltrosIDS=removefromfiltrosids(Plantas.FiltrosIDS,'".$filtronome."') WHERE FiltrosIDS LIKE '%".$filtronome.";%' OR FiltrosIDS LIKE '%".$filtronome."'";
...
if ($final==1) {
	...
	if ($nspecs>=1) {
		...
		foreach ($specarr as $spid) {
			$sql = "UPDATE Especimenes SET Especimenes.FiltrosIDS=IF(Especimenes.FiltrosIDS<>'',CONCAT(Especimenes.FiltrosIDS,';','".$filtronome."'),'".$filtronome."') WHERE EspecimenID='".$spid."'";
			mysql_query($sql,$conn);
		}
	} elseif ($nspecs==0) {
		$qq = "DELETE FROM Filtros WHERE FiltroID=".$_POST['filtro'];
...
<thead>
<tr><td colspan=100%>".GetLangVar('nameeditar')." ".GetLangVar('namefiltro')."s
...
<td colspan=100%><table><tr><td class='tdsmallbold'>".GetLangVar('namefiltro')."</td>
	...
	echo "<form name='finalform' action=filtro-edit.php method='post'>
