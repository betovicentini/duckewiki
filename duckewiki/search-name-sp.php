...
HTMLheaders('');
		...
		//taxonomic summary
			//taxonomic summary		
		$qq = "SELECT DISTINCT Genero,Especie,InfraEspecie FROM Tax_InfraEspecies JOIN Tax_Especies USING(EspecieID) JOIN Tax_Generos USING(GeneroID) JOIN Tax_Familias USING(FamiliaID) WHERE Especie='".$nn."' AND Genero='".$gen."'";
		//echo $qq;
		$rr = mysql_query($qq,$conn);
		$ninfsp = mysql_numrows($rr);
		
		$qq = "SELECT * FROM Especimenes JOIN Identidade USING(DetID) JOIN Tax_Especies ON Identidade.EspecieID=Tax_Especies.EspecieID JOIN Tax_Generos ON Tax_Especies.GeneroID=Tax_Generos.GeneroID WHERE Especie='".$nn."' AND Genero='".$gen."'";
		$rr = mysql_query($qq,$conn);
		$ncols = mysql_numrows($rr); 
		
		$qq = "SELECT * FROM Plantas JOIN Identidade USING(DetID) JOIN Tax_Especies ON Identidade.EspecieID=Tax_Especies.EspecieID JOIN Tax_Generos ON Tax_Especies.GeneroID=Tax_Generos.GeneroID WHERE Especie='".$nn."' AND Genero='".$gen."'";
		$rr = mysql_query($qq,$conn);
		$nplantas = mysql_numrows($rr); 
		
		//imagens
		$qq = "(SELECT TraitVariation as imagens FROM Traits_variation JOIN Traits USING (TraitID) JOIN Especimenes  ON Traits_variation.EspecimenID=Especimenes.EspecimenID JOIN Identidade USING(DetID) JOIN Tax_Especies ON Identidade.EspecieID=Tax_Especies.EspecieID JOIN Tax_Generos ON Tax_Especies.GeneroID=Tax_Generos.GeneroID WHERE Especie='".$nn."' AND Genero='".$gen."' AND TraitTipo='Variavel|Imagem') UNION (SELECT TraitVariation as imagens FROM Traits_variation JOIN Traits USING (TraitID) JOIN Plantas ON Traits_variation.PlantaID=Plantas.PlantaID JOIN Identidade USING(DetID) JOIN Tax_Especies ON Identidade.EspecieID=Tax_Especies.EspecieID JOIN Tax_Generos ON Tax_Especies.GeneroID=Tax_Generos.GeneroID WHERE Especie='".$nn."' AND Genero='".$gen."' AND TraitTipo='Variavel|Imagem') UNION (SELECT TraitVariation as imagens FROM Traits_variation JOIN Traits USING (TraitID) JOIN Tax_Especies ON Traits_variation.EspecieID=Tax_Especies.EspecieID JOIN Tax_Generos ON Tax_Especies.GeneroID=Tax_Generos.GeneroID WHERE Especie='".$nn."' AND Genero='".$gen."' AND TraitTipo='Variavel|Imagem')";
		...
		$qq = "SELECT EspecieID,Especie,Tax_Especies.BasionymAutor,EspecieAutor,Genero,Tax_Especies.Sinonimos as Sinonimos,Tax_Especies.Notas,Tax_Especies.GeoDistribution,Tax_Especies.PubRevista,Tax_Especies.PubVolume,Tax_Especies.PubAno,Familia,Tax_Especies.Valid,Ordem FROM Tax_Especies JOIN Tax_Generos USING(GeneroID) JOIN Tax_Familias USING(FamiliaID) WHERE Especie='".$nn."' AND Genero='".$gen."'";
		...
		if ($rr) {
			...
			$ordem = $row['Ordem'];			
			$fam = $row['Familia'];
			$spaut1 = $row['BasionymAutor'];
			$spauto2 = $row['EspecieAutor'];			
			...
			if ($valid==1) {
				...
				$qq = "(SELECT Genero as nome  FROM Tax_Generos WHERE Sinonimos LIKE '%especie|".$speciesid.";%' OR Sinonimos LIKE '%especie|".$speciesid."') UNION (SELECT concat(Genero,\" \",Especie) as nome  FROM Tax_Especies JOIN Tax_Generos USING(GeneroID) WHERE Tax_Especies.Sinonimos LIKE '%especie|".$speciesid.";%' OR Tax_Especies.Sinonimos LIKE '%especie|".$speciesid."')";
		...
		if ($valid==1) {$namevalid = strtolower(GetLangVar('namevalido'));}
		if ($valid==0) {$namevalid = strtolower(GetLangVar('nameinvalido'));}
		
		$orderurl = "http://www.mobot.org/mobot/research/apweb/orders/".strtolower($ordem)."web.htm#".$ordem;
		$famurl = "http://www.mobot.org/mobot/research/apweb/orders/".strtolower($ordem)."web.htm#".$fam;
		$resurl = file_exists($orderurl);
		if (!$resurl) {
			$orderurl = "http://www.mobot.org/mobot/research/apweb/orders/".strtolower($ordem)."web2.htm#".$ordem;
			$famurl = "http://www.mobot.org/mobot/research/apweb/orders/".strtolower($ordem)."web2.htm#".$fam;
		...
		<tr >
			<td  colspan=2 class='tabhead'><i>$gen&nbsp;$nn<i></td>
		</tr>
		<tr>";
			if ($valid==1) {
			...
			echo "
				<tr>
					...
					<td class='tdsmallbold'><i>$gen&nbsp;$nn</i>&nbsp;$spaut1&nbsp;$spauto2&nbsp; ($famlink | $orderlink)&nbsp;";
					if ($valid==0) {
						echo "[$namevalid]";					
					}
					echo "</td>
				...
				<tr>
					...
					if ($valid==0) {
						echo " [".strtolower(GetLangVar('namevalido'))."]";					
					}
					echo "</td>
				</tr>	
				<tr>
		...
		<tr>
			<td align='center'>";
			if ($ninfsp>0) { 
				echo "<a href=\"?gen=$gen&nn=$nn&listinfsp=1\">".$ninfsp."</a>";
			} else {
				echo $nifnsp;
			}
			echo "</td>
			...
			<td align='center'>";
			if ($ncols>0) {
					echo "<a href=\"?gen=$gen&nn=$nn&listcol=1\">".$ncols."</a>";
				} else { echo $ncols;}
			echo "			
			</td>";
			if ($listplantas==1) { echo "<td align='center' class='clicked'>"; } else { echo "<td align='center'>";}
			if ($nplantas>0) {
				echo "<a href=\"?gen=$gen&nn=$nn&listplantas=1\">".$nplantas."</a>";
			} else {
				echo $nplantas;
			}
			echo "</td>
		...
		if ($listinfsp==1 && $ninfsp>0) {
			echo "
				...
				<tr>
					...
					<tbody >";			
				$qq = "SELECT Genero,Especie,EspecieAutor,Tax_Especies.BasionymAutor as SpBasAu,InfraEspecieNivel as nivel,InfraEspecie,InfraEspecieAutor as autor,Tax_InfraEspecies.BasionymAutor FROM Tax_InfraEspecies JOIN Tax_Especies USING(EspecieID) JOIN Tax_Generos USING(GeneroID) JOIN Tax_Familias USING(FamiliaID) WHERE Especie='".$nn."' AND Genero='".$gen."' ORDER BY Genero,Especie,InfraEspecie";
				$rr = mysql_query($qq,$conn);
				$bgi=1;
