CREATE FUNCTION traitvariation_plantas(ttid int(10), pltid int(10), trunit BOOLEAN, somedia BOOLEAN) RETURNS VARCHAR(500)
BEGIN
...
DECLARE cur1 CURSOR FOR SELECT TraitVariation,Traits_variation.TraitUnit,Traits.TraitTipo FROM Traits_variation JOIN Traits USING(TraitID) WHERE TraitID=ttid AND PlantaID=pltid;
