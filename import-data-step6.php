...
if (in_array('MAJORAREA',$fields) && in_array('COUNTRY',$fields) && !isset($provinciasdone)) {
		$colcol = array_search('MAJORAREA',$fields);
		if (count($colcol)>1) {
			$erro = '
            <br />Tem mais de uma coluna que define majorarea??';
		} else {
			$clnl2 = $tbprefix."ProvinceID";
			...
			$qq = "SELECT DISTINCT ".$colcol." as missgen,".$clnl." FROM ".$tbname." WHERE ".$clnl2."=0 AND ".$colcol."<>'' AND (".$colcol." IS NOT NULL) ";
...
<table align='center' class='myformtable' cellpadding='7'>
  <thead>
    <tr><td colspan='100%'>MajorArea (".$colcol.") não encontrados no Wiki</td></tr>
    <tr class='subhead'>
      <td>Nome</td>
  ...
  <tbody>
  <form action='import-data-step6.php' method='post'>";
					unset($_POST['provincedone']);
...
";
	...
	elseif (!in_array('MAJORAREA',$fields)) { 
		$provinciasdone = 1;
}
