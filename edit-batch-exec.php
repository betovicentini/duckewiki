...
if ($final==1) {
	...
	if ($filtro>0 && !isset($page)) { 
		$specarr = array();
		$qq = '';
		$qq  = "SELECT EspecimenID FROM Especimenes JOIN Pessoas ON ColetorID=PessoaID WHERE FiltrosIDS LIKE '%filtroid_".$filtro.";%' OR FiltrosIDS LIKE '%filtroid_".$filtro."'";
	...
	foreach ($specarr as $i) {
		...
		<tr>
		    ...
		    <table>
				...
				echo "</select></td>
				<td style='border: 0px'>".strtolower(GetLangVar('nameor'))."</td>
				<td align='left' style='border: 0px' >
