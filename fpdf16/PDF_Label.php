...
class PDF_Label extends FPDF {
    ...
    var $_Avery_Labels = array(
        ...
        'L7651'=> array('paper-size'=>'A4',        'metric'=>'mm',    'marginLeft'=>6.1,      'marginTop'=>10.9,        'NX'=>5,    'NY'=>13,   'SpaceX'=>1.375,    'SpaceY'=>0,    'width'=>38.1,       'height'=>21.2,        'font-size'=>7)
    );
    ...
    // Print a label
    function Add_Label_Tag($taggedtext) {
        $this->_COUNTX++;
        if ($this->_COUNTX == $this->_X_Number) {
            // Row full, we start a new one
            $this->_COUNTX=0;
        ...
        $_PosY = $this->_Margin_Top + $this->_COUNTY*($this->_Height+$this->_Y_Space) + $this->_Padding;
        $this->SetXY($_PosX, $_PosY);
        $this->MultiCellTag($this->_Width - $this->_Padding, $this->_Line_Height, $taggedtext, 0, 'L');
    }
    
	function Add_LabelQRCode($text,$w,$h,$ty) {
	 	$this->_COUNTX++;
        if ($this->_COUNTX == $this->_X_Number) {
            // Row full, we start a new one
            $this->_COUNTX=0;
        ...
        $_PosY = $this->_Margin_Top + $this->_COUNTY*($this->_Height+$this->_Y_Space) + $this->_Padding;
        $this->SetXY($_PosX, $_PosY);
        //$this->GDImage($im, $_PosX,$_PosY,$w,$h,''); 
        $this->Image($text,$_PosX,$_PosY,$w,$h,$ty);
	}

	function Add_ImageLabelWide($pathimg,$w,$h,$ty,$text,$ftsize) {
	 	$this->_COUNTX++;
        if ($this->_COUNTX == $this->_X_Number) {
            // Row full, we start a new one
            $this->_COUNTX=0;
        ...
        $this->Image($pathimg,$_PosX,$_PosY,$w,$h,$ty,'');
        $this->SetXY($_PosX, $_PosY+$h);
        $this->Set_Font_Size($ftsize);
        $this->MultiCell($this->_Width - $this->_Padding, $this->_Line_Height, $text, 0, 'L');
	}
