...
echo "
	...
	if (empty($tbname)) {
		if ($sampletype=='specimens' || $processoid>0) {
			if (($processoid+0)==0) {
				$qq = "SELECT CONCAT('Spec','-',EspecimenID) as WikiID, CONCAT(pess.Sobrenome,'-',Number) as TAG FROM Especimenes JOIN Pessoas as pess ON pess.PessoaID=ColetorID" ;
				$qq .= " WHERE FiltrosIDS LIKE '%filtroid_".$filtro.";%'  OR FiltrosIDS LIKE '%filtroid_".$filtro."' ".$qo;
			} else {
				$qq = "SELECT CONCAT('Spec','-',prcc.EspecimenID) as WikiID, CONCAT(pess.Sobrenome,'-',Number) as TAG FROM ProcessosLIST as prcc JOIN Especimenes ON prcc.EspecimenID=Especimenes.EspecimenID JOIN Pessoas as pess ON pess.PessoaID=Especimenes.ColetorID WHERE prcc.EXISTE=1 AND prcc.ProcessoID=".$processoid;
			}
			...
			$qq .= " WHERE pl.FiltrosIDS LIKE '%filtroid_".$filtro.";%'  OR pl.FiltrosIDS LIKE '%filtroid_".$filtro."' ".$qo;
		}
	} else {
